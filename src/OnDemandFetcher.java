...
public final class OnDemandFetcher extends OnDemandFetcherParent implements
	...
	/*
	 * public void start(StreamLoader streamLoader, client client1) { byte[]
	 * abyte2 = streamLoader.getDataForName("map_index"); Stream stream2 = new
	 * Stream(abyte2); int j1 = abyte2.length / 7; mapIndices1 = new int[j1];
	 * mapIndices2 = new int[j1]; mapIndices3 = new int[j1]; mapIndices4 = new
	 * int[j1]; for (int i2 = 0; i2 < j1; i2++) { mapIndices1[i2] =
	 * stream2.readUnsignedWord(); mapIndices2[i2] = stream2.readUnsignedWord();
	 * mapIndices3[i2] = stream2.readUnsignedWord(); mapIndices4[i2] =
	 * stream2.readUnsignedByte(); }
	 * 
	 * abyte2 = streamLoader.getDataForName("midi_index"); stream2 = new
	 * Stream(abyte2); j1 = abyte2.length; anIntArray1348 = new int[j1]; for
	 * (int k2 = 0; k2 < j1; k2++) anIntArray1348[k2] =
	 * stream2.readUnsignedByte();
	 * 
	 * clientInstance = client1; running = true;
	...
	public void run() {
		try {
			...
			signlink.reporterror("Cannot Find Something"); // od_ex
	...
	public void handleFailed() {
		...
		while (uncompletedCount < 10) {
			...
			if (onDemandData_1 == null)
				break;
			System.out.println(onDemandData_1.dataType + ", " + onDemandData_1.ID);
			//missingFile(onDemandData_1);
			try{
	...
	public void missingFile(OnDemandData demand) {
		String[] archives = { "Model", "Animation", "Music", "Map" };
		
		System.out.println("Couldn't find file: " + demand.ID + " in " + archives[demand.dataType - 1] + " archive.");
	}
