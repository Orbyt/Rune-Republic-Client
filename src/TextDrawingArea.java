...
public final class TextDrawingArea extends DrawingArea {
	...
	public int getColorByID(String s) {
		if (s.contains("col=")) {
			String color = s.substring(s.indexOf("=") + 1, 10);
			int finalColor = Integer.valueOf(color, 16).intValue();
			return finalColor;
		}
		return -1;
	}
	...
	public void method3822(int i, int j, int k, String s, int l, boolean flag) {
		k = 74 / k;
		method389(flag, j - getTextWidth(s) / 2, i, s, l);
	}
...
try {	
	...
	private int getColorByName(String s) {
		...
		if (s.equals("gr3"))
			return 0x40ff00;
		/** Loyalty Colors **/
		if (s.equals("bas")) { //you can use this to change the color of in game titles
			return 0xC86400;
		}
		if (s.equals("mbl")) {
			return 0xAF64A0;
