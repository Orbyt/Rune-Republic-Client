...
public class DrawingArea extends NodeSub {
	...
	public static void drawPixels(int dotHeight, int dotY, int dotX, int dotColor, int dotWidth)
	{
		if(dotX < topX)
		{
			dotWidth -= topX - dotX;
		...
		if(dotX + dotWidth > bottomX)
			dotWidth = bottomX - dotX;
		if(dotY + dotHeight > bottomY)
			dotHeight = bottomY - dotY;
		int k1 = width - dotWidth;
		int l1 = dotX + dotY * width;
		for(int i2 = -dotHeight; i2 < 0; i2++)
		{
			for(int j2 = -dotWidth; j2 < 0; j2++)
				pixels[l1++] = dotColor;
	...
	public static void drawPixelsWithOpacity(int color, int yPos,
			int pixelWidth, int pixelHeight, int opacityLevel, int xPos) {
		if (xPos < topX) {
			pixelWidth -= topX - xPos;
			xPos = topX;
		}
