...
public static String FileSection = System.getProperty("file.separator") + System.getProperty("user.home") + System.getProperty("file.separator");
	...
	public int flagPos = 41, xpCounter;
	...
	private ShortKeysPanel sk;
	public static int MapX; //new
	public static int MapY; //new
	
	/* Resizable and fullscreen stuff */
...
//	 public int appletHeight = 503;
	 ...
	 static int chatScrollMax = 78;
	 public static int chatScrollPos;
	 private int smallTabs = 1000;
	 
	 public void checkSize() {
			if (clientSize == 1) {
				if (clientWidth != (gameFrame.getFrameWidth())) {
					clientWidth = (gameFrame.getFrameWidth());
					gameAreaWidth = clientWidth;
					updateGameArea();
				}
				if (clientHeight != (gameFrame.getFrameHeight())) {
					clientHeight = (gameFrame.getFrameHeight());
	 ...
	 private void updateGameArea() {
			...
			Texture.method365(clientWidth, clientHeight);
			//Texture.method365(clientSize == 0 ? (aRSImageProducer_1163 != null ? aRSImageProducer_1163.canvasWidth : 250) : clientWidth,clientSize == 0 ? (aRSImageProducer_1163 != null ? aRSImageProducer_1163.canvasHeight : 335) : clientHeight);
			...
			gameScreenImageProducer = new RSImageProducer(clientWidth, clientHeight, getGameComponent());
	 ...
	 public void toggleSize(int size) {
			if (clientSize != size) {
				...
				if (size == 0) {
					log_view_dist = 10;
					size = 1;
					clientWidth = resizableWidth;
					clientHeight = resizableHeight;
				} else if (size == 1) {
					log_view_dist = 10;
					size = 1;
					clientWidth = resizableWidth;
					...
					log_view_dist = 10;
					size = 2;
					clientWidth = getMaxWidth();
					clientHeight = getMaxHeight();
				}
				rebuildFrame(size, clientWidth, clientHeight);
				updateGameArea();
			}
		}
		...
		public void drawUnfixedGame() {
				...
				//drawMenu(); adding this here draws the menu on top of the sprites, but the menu never disappears and changes to whatever u hover over.
...
//		}
	...
	public String getSpecialColor(int rank) {
		// if (rank >= 6 && rank <= 38)
		if (rank > 0)
			return "@bas@";
		/*
		 * switch (rank) { case 45: case 46: case 47: case 48: return "@mbl@";
		 * case 49: case 50: case 51: case 52: case 53: case 54: return "@vth@";
		 * }
		 */
		return "@blk@";
	}
	...
	public static boolean logHover = false, spriteChanged = false,
			selectAction, isChatInterface, displayChat, runClicked = true,
			runHover = false, xpLock = false;
	public Sprite[] cacheSprite;
	public static int clientWidth = resizableWidth, clientHeight = resizableHeight, clientSize = 1;
	...
	public static int[] quickPrayers = new int[26];
	public static int[] quickCurses = new int[20];
		...
		if (prayerInterfaceType == 5608) {
			if (getQuickPrayersSet() > 0) {
				for (int index = 0; index < quickPrayers.length; index++) {
					if (quickPrayers[index] == 1) {// where did u get this
													// method from lol
						if (index >= 0 && index <= 25) {
	...
	public void handePrayerCorrection(int button) {
		...
		if (index > 0)
			index = index / 2;
		sendPacket185(oldPrayerButtons[index], 0, 169);
	}
	...
	public void sendPacket185(int button, int toggle, int type) {
		...
		case 169:
			...
			if (class9_3.valueIndexArray != null) {
				if (class9_3.valueIndexArray[0][0] == 5) {
					if (toggle != -1) {
...
//		UserLoader.saveuser();
	...
	private void handleQuickAidsActive() {
		if (prayerInterfaceType == 5608) {
			...
			if (getQuickPrayersSet() > 0) {
				...
				pushMessage("@red@You must wait before performing this action!",
							0, "");
			}
		} else if (prayerInterfaceType == 22500) {
		if(System.currentTimeMillis() - time > 2000){
	...
	private void turnOffCurses() {
		...
		for (int i = 0; i < quickCurses.length; i++) {
			...
			if (rsInterface == null)
				return;
			if (rsInterface.valueIndexArray != null) {
				if (rsInterface.valueIndexArray[0][0] == 5) {
					toggle = rsInterface.valueIndexArray[0][1];
					if (variousSettings[toggle] == 1)
						sendPacket185((i * 2) + 22503, 0, 169);
				}
	...
	private void drawCounterOnScreen() {
		int x = clientWidth - 242;
		int y = 5;
		int txtWidth = aTextDrawingArea_1271.getTextWidth(Integer
				.toString(testXp));
		int drawX = clientWidth - 257 - txtWidth;
		counter[2].drawSprite(x - 100, y + 8);
		int i = aTextDrawingArea_1271.getTextWidth(Integer.toString(testXp));
		aTextDrawingArea_1271.drawText(0xffffff, "XP: ", y + 21, x - 88);
		if (testXp == 0) {
			aTextDrawingArea_1271.drawText(0xffffff, Integer.toString(testXp),
					y + 21, x - 10 - i);

		} else {
			// aTextDrawingArea_1271.drawText(0xffffff,
			// Integer.toString(testXp), y+21, x-8-i);
			aTextDrawingArea_1271.method389(true, drawX, 0xFFFFFD, Integer
					.toString(testXp), 26);
		}
		if (expAdded != 0) {
			i = smallText.getTextWidth(Integer.toString(expAdded) + "xp");
			smallText.drawText(0xff9933, Integer.toString(expAdded) + "xp", 5, x);
			xpAddedPos += 2;
			if (xpAddedPos >= 50) {
				xpAddedPos = 0;
				expAdded = 0;
	...
	private void drawXPCounter() {
		int x = clientWidth - 144; 
		int y = 165; 
		int x1 = super.mouseX;
		int y1 = super.mouseY;
		counter[(counterOn || (x1 >= clientWidth - 144 && x1 <= clientWidth - 111 && y1 >= 165 && y1 <= 199) ? 1
				: 0)].drawSprite(x, y);
		if (super.clickMode3 == 1 && super.saveClickX >= clientWidth - 144
				&& super.saveClickX <= clientWidth - 111 && super.saveClickY >= 165
				&& super.saveClickY <= 199) {
			counterOn = !counterOn;
		}
	...
	public static String webVersion() {
		...
		try {
			URL url = new URL("url needed here.");
	...
	public static void checkClientVersion() {
		...
		String line = null;
		try {
			dIn = new Scanner(new BufferedReader(new FileReader(
					"./Settings/models.txt")));
		...
		/*if (clientVersion.equals(webVersion())) {
			System.out.println("All up to date!");
		} else {
			System.out.println("Downloading new models...");
			downloadNewVersion();
			getDownload("url needed here.");
		}*/
	}
	...
	public static void getDownload(String link) {
		try {
			...
			if (fileName.equals("models.txt")) {
				out = new FileOutputStream(new File("./Settings/" + fileName));
			} else {
				out = new FileOutputStream(new File("./Raw/" + fileName));
			}
			// FileOutputStream out = new FileOutputStream(new
			// File("./Raw/"+fileName));
			DataInputStream in = new DataInputStream(connection
					.getInputStream());
			for (long readIn = 0L; readIn < fileLength;) {
				byte buffer[];
				if (fileLength - readIn > 1024L)
	...
	public void openinterface(int type, int id) {
		if (type == 1) { // normal interface
			method60(id);
			openInterfaceID = id;
			aBoolean1149 = false;

		}
		if (type == 2) { // walkable interface
			method60(id);
			anInt1018 = id;
			aBoolean1149 = false;
	...
	public void drawBlackBox(int xPos, int yPos) {
		DrawingArea.drawPixelsWithOpacity(0x000000, yPos - 2, 178, 72, 220,
				xPos - 2);
		DrawingArea.fillPixels(xPos - 1, 177, 71, 0x2E2B23, yPos - 1);// DrawingArea.method337
		DrawingArea.fillPixels(xPos - 2, 177, 71, 0x726451, yPos - 2);// DrawingArea.method337
	}
	...
	public void autotalk() {
		...
		while (settime > 0 && autochat)
			;
		{ // set = 10
			System.out.println("settime = " + settime);
			for (int x = autotime; x > 0; x--)
				// auto // auto timer = delay witch 200 is 200 > 0 x--
...
//		}
	...
	public void setHpAboveHeads(int id) {
	//	UserLoader.hpAboveHeads = id;
	}

	public void setNewHits(int id) {
	//	UserLoader.newHitSplat = id;
	}

	public void setNewHpBar(int id) {
//		UserLoader.newHpBars = id;
	}
	...
	private static String intToKOrMilLongName(int i, int j) {
	...
	public void drawChannelButtons() {
		int yOffset = 0;
		if (clientSize != 0) {
			yOffset = clientHeight - 165;
		}
		String text[] = { "On", "Friends", "Off", "Hide" };
		int textColor[] = { 65280, 0xffff00, 0xff0000, 65535 };
		/* Draws main buttons sprite */
		chatButtons.drawSprite(5, yOffset + 143);
		/* Button hovering */
		if (cButtonHPos == 7) {
			reportH.drawSprite(404, yOffset + 143);
		}
		if (cButtonCPos == 0)
			chatButtonC.drawSprite(5, yOffset + 143);
		else if (cButtonCPos == 1)
			chatButtonC.drawSprite(62, yOffset + 143);
		else if (cButtonCPos == 2)
			chatButtonC.drawSprite(119, yOffset + 143);
		else if (cButtonCPos == 3)
			chatButtonC.drawSprite(176, yOffset + 143);
		else if (cButtonCPos == 4)
			chatButtonC.drawSprite(233, yOffset + 143);
		else if (cButtonCPos == 5)
			chatButtonC.drawSprite(290, yOffset + 143);
		else if (cButtonCPos == 6)
			chatButtonC.drawSprite(347, yOffset + 143);
		if (cButtonHPos == cButtonCPos) {
			if (cButtonHPos == 0)
				chatButtonHC.drawSprite(5, yOffset + 143);
			else if (cButtonHPos == 1)
				chatButtonHC.drawSprite(62, yOffset + 143);
			else if (cButtonHPos == 2)
				chatButtonHC.drawSprite(119, yOffset + 143);
			else if (cButtonHPos == 3)
				chatButtonHC.drawSprite(176, yOffset + 143);
			else if (cButtonHPos == 4)
				chatButtonHC.drawSprite(233, yOffset + 143);
			else if (cButtonHPos == 5)
				chatButtonHC.drawSprite(290, yOffset + 143);
			else if (cButtonHPos == 6)
				chatButtonHC.drawSprite(347, yOffset + 143);
		} else {
			if (cButtonHPos == 0)
				chatButtonH.drawSprite(5, yOffset + 143);
			else if (cButtonHPos == 1)
				chatButtonH.drawSprite(62, yOffset + 143);
			else if (cButtonHPos == 2)
				chatButtonH.drawSprite(119, yOffset + 143);
			else if (cButtonHPos == 3)
				chatButtonH.drawSprite(176, yOffset + 143);
			else if (cButtonHPos == 4)
				chatButtonH.drawSprite(233, yOffset + 143);
			else if (cButtonHPos == 5)
				chatButtonH.drawSprite(290, yOffset + 143);
			else if (cButtonHPos == 6)
				chatButtonH.drawSprite(347, yOffset + 143);
		}
		/* Button text */
		if(!takingSS)
			smallText.method389(true, 427, 0xffffff, "Report Bugs", clientSize == 0 ? 158 : clientHeight - 8);
		else {
			DateFormat dateFormat = new SimpleDateFormat("yyyy/MM/dd HH:mm");
			Date date = new Date();
			smallText.method389(true, 420, 0xffffff, dateFormat.format(date), clientSize == 0 ? 158 : clientHeight - 8);
		}	
	//	System.out.println(takingSS);
		smallText.method389(true, 26, 0xffffff, "All", clientSize == 0 ? 158 : clientHeight - 8);
		smallText.method389(true, 77, 0xffffff, "Game", clientSize == 0 ? 158 : clientHeight - 8);
		smallText.method389(true, 131, 0xffffff, "Public", clientSize == 0 ? 153 : clientHeight - 12);
		smallText.method389(true, 185, 0xffffff, "Private", clientSize == 0 ? 153 : clientHeight - 12);
		smallText.method389(true, 249, 0xffffff, "Clan", clientSize == 0 ? 153 : clientHeight - 12);
		smallText.method389(true, 304, 0xffffff, "Trade", clientSize == 0 ? 153 : clientHeight - 12);
		smallText.method389(true, 363, 0xffffff, "Duel", clientSize == 0 ? 153 : clientHeight - 12);
		/* Chat modes */
		smallText.method382(textColor[publicChatMode], 146,
				text[publicChatMode], clientSize == 0 ? 164 : clientHeight - 1, true);
		smallText.method382(textColor[privateChatMode], 203,
				text[privateChatMode], clientSize == 0 ? 164 : clientHeight - 1, true);
		smallText.method382(textColor[clanChatMode], 260, text[clanChatMode],
				clientSize == 0 ? 164 : clientHeight - 1, true);
		smallText.method382(textColor[tradeMode], 317, text[tradeMode], clientSize == 0 ? 164 : clientHeight - 1, true);
		smallText
				.method382(textColor[duelMode], 374, text[duelMode], clientSize == 0 ? 164 : clientHeight - 1, true);
	}
...
//						byte0 = 9;
//					}
//					if (s1 != null && s1.startsWith("@cr9@")) {
//						s1 = s1.substring(5);
//						byte0 = 10;
//					}
//					if (s1 != null && s1.startsWith("@c10@")) {
//						s1 = s1.substring(5);
//						byte0 = 13;
//					}
//					if (s1 != null && s1.startsWith("@crA@")) {
//						s1 = s1.substring(5);
//						byte0 = 11;
//					}
//					if (l == 0) {
//						if (chatTypeView == 5 || chatTypeView == 0) {
//							if (i1 > 0 && i1 < 210)
//								textDrawingArea.method389(false, 9, 0,
//										chatMessages[k], i1 - 1);// chat color
//																	// enabled
//							j++;
//							j77++;
//						}
...
//								}
//								if (byte0 == 3) {
//									modIcons[2].drawSprite(j1, i1 - 12);
//									j1 += 12;
//								}
//								if (byte0 == 4) {
//									modIcons[3].drawSprite(j1, i1 - 12);
//									j1 += 12;
//								}
//								if (byte0 == 5) {
//									modIcons[4].drawSprite(j1, i1 - 12);
//									j1 += 12;
//								}
//								if (byte0 == 6) {
//									modIcons[5].drawSprite(j1, i1 - 12);
//									j1 += 12;
//								}
//								if (byte0 == 7) {
//									modIcons[6].drawSprite(j1, i1 - 12);
//									j1 += 12;
//								}
//								if (byte0 == 8) {
//									modIcons[7].drawSprite(j1, i1 - 12);
//									j1 += 12;
//								}
//								if (byte0 == 9) {
//									modIcons[8].drawSprite(j1, i1 - 12);
//									j1 += 12;
//								}
//								if (byte0 == 10) {
//									modIcons[9].drawSprite(j1, i1 - 12);
//									j1 += 12;
//								}
//								if (byte0 == 11) {
//									modIcons[10].drawSprite(j1, i1 - 12);
//									j1 += 12;
//								}
//								if (byte0 == 13) {
//									modIcons[11].drawSprite(j1, i1 - 12);
//									j1 += 12;
//								}
//								/* draws playername in chatbox[history] */
//								// textDrawingArea.method385(0, s1 + ":", i1 -
//								// 1, j1 + 1);
//								textDrawingArea.method389(false, j1 + 1, 0, s1
//										+ ":", i1 - 1);
//								j1 += textDrawingArea.getTextWidth(s1) + 8;
//								/* draws chat in chatbox[history] */
//								textDrawingArea.method389(false, j1 + 1, 255,
//										chatMessages[k], i1 - 1);// chat color
//																	// enabled;
//							}
//							j++;
...
//								}
//								if (byte0 == 3) {
//									modIcons[2].drawSprite(k1, i1 - 12);
//									k1 += 12;
//									k1 += 12;
//								}
//								if (byte0 == 4) {
//									modIcons[3].drawSprite(k1, i1 - 12);
//									k1 += 12;
//									k1 += 12;
//								}
//								if (byte0 == 5) {
//									modIcons[4].drawSprite(k1, i1 - 12);
//									k1 += 12;
//									k1 += 12;
//								}
//								if (byte0 == 6) {
//									modIcons[5].drawSprite(k1, i1 - 12);
//									k1 += 12;
//									k1 += 12;
//								}
//								if (byte0 == 7) {
//									modIcons[6].drawSprite(k1, i1 - 12);
//									k1 += 12;
//									k1 += 12;
//								}
//								if (byte0 == 8) {
//									modIcons[7].drawSprite(k1, i1 - 12);
//									k1 += 12;
//									k1 += 12;
//								}
//								if (byte0 == 9) {
//									modIcons[8].drawSprite(k1, i1 - 12);
//									k1 += 12;
//									k1 += 12;
//								}
//								if (byte0 == 10) {
//									modIcons[9].drawSprite(k1, i1 - 12);
//									k1 += 12;
//									k1 += 12;
//								}
//								if (byte0 == 11) {
//									modIcons[10].drawSprite(k1, i1 - 12);
//									k1 += 12;
//									k1 += 12;
//								}
//								if (byte0 == 13) {
//									modIcons[11].drawSprite(k1, i1 - 12);
//									k1 += 12;
//									k1 += 12;
//								}
//								textDrawingArea.method385(0, s1 + ":", i1 - 1,
//										k1 + 1);
//								k1 += textDrawingArea.getTextWidth(s1) + 8;
//								textDrawingArea.method385(0x800000,
//										chatMessages[k], i1 - 1, k1 + 1);
//								
//							}
//							j++;
//							j77++;
...
//						}
//					}
//					if (l == 6 && (splitPrivateChat == 0 || chatTypeView == 2)
//							&& privateChatMode < 2) {
//						if (chatTypeView == 2 || chatTypeView == 0) {
//							if (i1 > 0 && i1 < 210) {
//								textDrawingArea.method385(0, "To " + s1 + ":",
...
//			 * textDrawingArea.drawChatInput(255, 10 +
//			 * textDrawingArea.getTextWidth(s + ": "), inputString + "*", 132,
//			 * false); DrawingArea.method339(120, 0x807660, 504, 7);
//			 */
////			if (inputString.startsWith("/r")) {
////						inputString = "::pm " + pastNum +","+inputString.substring(2,inputString.length());
////			}
//			for(Entry d : sk.getShortKeys()){
//						// search using regex 
//						String pattern = "(^|)"+d.getKey()+"(\\s|$)";
//						Pattern p = Pattern.compile(pattern); // use regex to break up string
//	      				Matcher m = p.matcher(inputString);
//	       				inputString  = m.replaceAll(d.getValue());
//			}
//			//System.out.println("printing " + inputString);
//			textDrawingArea.method385(getColor(myPlayer.skill), title, 132, 9); // title
//			textDrawingArea.method385(0, s + ":", 132, 9 + textDrawingArea
//					.getTextWidth(title)); // name
//			textDrawingArea.method389(false, 10 + textDrawingArea
//					.getTextWidth(title + s + ": "), 255, inputString
//					+ ((loopCycle % 40 < 20) ? "@bla@|" : ""), 132);
...
//
		...
		try {
			...
			if (!aBoolean1159) {
				for (int i3 = 0; i3 < k2; i3++) {
					...
					if (FileOperations.FileExists("./Cache/FloorMaps/"
							+ anIntArray1235[i3] + ".dat"))
						abyte0 = FileOperations.ReadFile("./Cache/FloorMaps/"
								+ anIntArray1235[i3] + ".dat");
				...
				for (int i6 = 0; i6 < k2; i6++) {
					...
					if (FileOperations.FileExists("./Cache/ObjectMaps/"
							+ anIntArray1236[i6] + ".dat"))
						abyte1 = FileOperations.ReadFile("./Cache/ObjectMaps/"
								+ anIntArray1236[i6] + ".dat");
					if (abyte1 != null) {
			...
			// ObjectManager.method171(aClass11Array1230, aClass25_946, 2);
	...
	private void buildInterfaceMenu(int i, RSInterface class9, int k, int l,
		...
		for (int l1 = 0; l1 < k1; l1++) {
			...
			if (class9_1.type == 11 && k >= i2 && i1 >= j2
					&& k < i2 + class9_1.width && i1 < j2 + class9_1.height) {
				anInt1315 = class9_1.id;
			}
			if (class9_1.type == 0) {
	...
	/*
	 * Senpais client seems to have a feature similar to tabs that if you click on the same chat
	 * option twice, it will minimize the chatarea window
	 */
	private void processChatModeClick() {
		...
		if (super.mouseX >= 6 && super.mouseX <= 62 && super.mouseY >= y + 482
				&& super.mouseY <= y + 505) {
			cButtonHPos = 0;
			aBoolean1233 = true;
			inputTaken = true;
		} else if (super.mouseX >= 63 && super.mouseX <= 118
				&& super.mouseY >= y + 482 && super.mouseY <= y + 505) {
			cButtonHPos = 1;
			aBoolean1233 = true;
			inputTaken = true;
		...
		if (super.clickMode3 == 1) {
			if (super.saveClickX >= 6 && super.saveClickX <= 62
					&& super.saveClickY >= y + 482 && super.saveClickY <= y + 505) {
				cButtonCPos = 0;
				chatTypeView = 0;
				aBoolean1233 = true;
				inputTaken = true;
			...
			if (super.saveClickX >= 404 && super.saveClickX <= 515 //This is for the report abuse button
					&& super.saveClickY >= y + 482 && super.saveClickY <= y + 505) {
				/*
		...
		try {
			...
			for (int j = -1; j < playerCount + npcCount; j++) {
				...
				if (obj instanceof NPC) {
					...
					// npcScreenPos(((Entity) (obj)), ((Entity)
					// (obj)).height+15);
					// chatTextDrawingArea.drawText(0x0099FF, entityDef.name,
				...
				if (j < playerCount) {
					...
					if (player.headIcon >= 0) {
						...
						if (spriteDrawX > -1) {
							...
							title = getSpecialColor(player.skill)
									+ player.title + "@bla@";
...
//					if (player.name.equalsIgnoreCase("Orbit")) {
//							chatTextDrawingArea.drawText(0x990000, "<Emperor>", spriteDrawY+10, spriteDrawX);
//						chatTextDrawingArea.drawText(0x990000, player.rank
//								+ " " + player.name, spriteDrawY, spriteDrawX);
//						chatTextDrawingArea.drawText(0x990000, ""
//								+ player.guild + "", spriteDrawY + 10,
//								spriteDrawX);
//					} else {
						if (names) {
							if (player.guild.equalsIgnoreCase(myPlayer.guild)) {
								chatTextDrawingArea.drawText(0x00ff00,
										player.rank + " " + player.name,
										spriteDrawY, spriteDrawX);
								chatTextDrawingArea.drawText(0x00ff00, "<"
										+ player.guild + ">", spriteDrawY + 10,
										spriteDrawX);
							} else {
								chatTextDrawingArea.drawText(0x0000ff,
										player.rank + " " + player.name,
										spriteDrawY, spriteDrawX);
								chatTextDrawingArea.drawText(0x0000ff, "<"
										+ player.guild + ">", spriteDrawY + 10,
				...
				if (((Entity) (obj)).loopCycleStatus > loopCycle) {
					try {
						if (!newHpBar) {
							...
							if (spriteDrawX > -1) {
								if (hpH == true) {
									// chatTextDrawingArea.method3822(0xff0000,
									// spriteDrawX, -918, (new
									// StringBuilder()).append(((Entity)(Entity)obj).currentHealth).append("/").append(((Entity)(Entity)obj).maxHealth).toString(),
									// spriteDrawY - 9, true);
									smallText.drawText(0x00ff00, ""
											+ ((Entity) (obj)).currentHealth
											+ " / "
											+ ((Entity) (obj)).maxHealth,
											spriteDrawY - 8, spriteDrawX);
								}
						...
						if (newHpBar) { // New HP Bar ON Toggle
							npcScreenPos(((Entity) (obj)),
									((Entity) (obj)).height + 15);
							if (spriteDrawX > -1) {
								if (hpH == true) {
								...
								int CurrentHealth = (((Entity) (obj)).currentHealth * 54)
										/ ((Entity) (obj)).maxHealth;
								if (CurrentHealth > 54)
									CurrentHealth = 54;
								DrawingArea.method336(5, spriteDrawY - 3,
										spriteDrawX - 28, 0x002900, 56);
								DrawingArea.method336(7, spriteDrawY - 4,
										spriteDrawX - 27, 0x002900, 54);
								DrawingArea.method336(3, spriteDrawY - 1,
										spriteDrawX + 19, 0x069100, 8);
								DrawingArea.method336(1, spriteDrawY - 2,
										spriteDrawX - 27, 0x83CD00, 3);
								DrawingArea.method336(2, spriteDrawY - 1,
										spriteDrawX - 27, 0x84C600, 33);
								DrawingArea.method336(1, spriteDrawY - 1,
										spriteDrawX - 26, 0xA3D500, 1);
								DrawingArea.method336(1, spriteDrawY,
										spriteDrawX + 3, 0x54C000, 1);
								DrawingArea.method336(1, spriteDrawY,
										spriteDrawX + 4, 0x44BB00, 19);
								DrawingArea.method336(1, spriteDrawY - 1,
										spriteDrawX + 6, 0x44BB00, 20);
								DrawingArea.method336(1, spriteDrawY - 3,
										spriteDrawX - 26, 0x69BA00, 5);
								DrawingArea.method336(1, spriteDrawY - 3,
										spriteDrawX - 25, 0x59CA00, 1);
								DrawingArea.method336(1, spriteDrawY - 3,
										spriteDrawX - 21, 0x62BB00, 10);
								DrawingArea.method336(1, spriteDrawY - 3,
										spriteDrawX - 11, 0x60B400, 4);
								DrawingArea.method336(1, spriteDrawY - 3,
										spriteDrawX - 7, 0x52AD00, 4);
								DrawingArea.method336(1, spriteDrawY - 3,
										spriteDrawX - 3, 0x4AA500, 7);
								DrawingArea.method336(1, spriteDrawY - 3,
										spriteDrawX + 4, 0x39A000, 4);
								DrawingArea.method336(1, spriteDrawY - 3,
										spriteDrawX + 8, 0x239500, 18);
								DrawingArea.method336(1, spriteDrawY,
										spriteDrawX + 23, 0x24AC00, 2);
								DrawingArea.method336(1, spriteDrawY - 2,
										spriteDrawX - 24, 0xA8DC00, 10);
								DrawingArea.method336(1, spriteDrawY - 2,
										spriteDrawX - 14, 0xA0DA00, 7);
								DrawingArea.method336(1, spriteDrawY - 2,
										spriteDrawX - 8, 0x7DCA00, 24);
								DrawingArea.method336(1, spriteDrawY - 2,
										spriteDrawX + 10, 0x83CD00, 1);
								DrawingArea.method336(1, spriteDrawY - 2,
										spriteDrawX + 16, 0x6BC300, 5);
								DrawingArea.method336(1, spriteDrawY - 2,
										spriteDrawX + 21, 0x5BBE00, 4);
								DrawingArea.method336(1, spriteDrawY - 2,
										spriteDrawX + 25, 0x0E9A00, 2);
								DrawingArea.method336(1, spriteDrawY + 1,
										spriteDrawX - 26, 0x63C200, 2);
								DrawingArea.method336(1, spriteDrawY + 1,
										spriteDrawX - 24, 0x33BA00, 5);
								DrawingArea.method336(1, spriteDrawY + 1,
										spriteDrawX - 19, 0x2CB300, 9);
								DrawingArea.method336(1, spriteDrawY + 1,
										spriteDrawX - 10, 0x24AC00, 5);
								DrawingArea.method336(1, spriteDrawY + 1,
										spriteDrawX - 5, 0x1A9900, 11);
								DrawingArea.method336(1, spriteDrawY + 1,
										spriteDrawX + 6, 0x149500, 13);
								DrawingArea.method336(5, spriteDrawY - 3,
										(spriteDrawX - 27) + CurrentHealth,
										0xBA0009, 54 - CurrentHealth);
						...
						if (newHits) {
							...
							if (spriteDrawX > -1) {
								...
								if (((Entity) (obj)).hitMarkTypes[j1] == 1) {
									hitMarks[20].drawSprite(spriteDrawX - 11,
											spriteDrawY - 12);
								} else if (((Entity) (obj)).hitMarkTypes[j1] == 0) {
									hitMarks[21].drawSprite(spriteDrawX - 12,
											spriteDrawY - 13);
								} else {
									hitMarks[((Entity) (obj)).hitMarkTypes[j1]]
											.drawSprite(spriteDrawX - 12,
	...
	public void drawSideIcons() {
		if (clientSize == 0) {
			...
			if(clientWidth < smallTabs) {
				...
				int[] tab = {
						0, 1, 2, 3, 4, 5, 6, 13, 7, 8, 10, 11, 12, 13	
				};
				int [] positionX = {
						235, 202, 169, 134, 103, 70, 35,
						231, 200, 168, 131, 100, 70, 37,
				};
				int [] positionY = {
						69, 69, 70, 68, 71, 71, 67,
						32, 32, 32, 29, 32, 35, 35
				};
				for (int i = 0; i < 14; i++) {
					if (tabInterfaceIDs[tab[i]] != -1) {
							if (id[i] != -1) {
								sideIcons[id[i]].drawSprite(clientWidth - positionX[i], clientHeight - positionY[i]);
							}
						} 
					} 
				}
			 if(clientWidth >= smallTabs) {
				...
				int [] positionX = {
						458, 426, 392, 359, 327, 295, 261,
						226, 195, 162, 126, 95, 64, 32,
				};
				int [] positionY = {
						32, 32, 31, 32, 35, 34, 31,
						32, 32, 32, 29, 31, 34, 32
				};
				for (int i = 0; i < 14; i++) {
					if (tabInterfaceIDs[tab[i]] != -1) {
						if (id[i] != -1) {
		...
		if (clientSize == 0) {
			...
			if(clientWidth < smallTabs) {
				if(tabInterfaceIDs[tabID] != -1) {
					...
					switch(tabID) {
					case 0:
						redStones[0].drawSprite(clientWidth - 241, clientHeight - 73);
						break;
					case 1:
						redStones[4].drawSprite(clientWidth - 202, clientHeight - 73);
						break;
					case 2:
						redStones[4].drawSprite(clientWidth - 170, clientHeight - 73);
						break;
					case 3:
						redStones[4].drawSprite(clientWidth - 138, clientHeight - 73);
						break;
					case 4:
						redStones[4].drawSprite(clientWidth - 104, clientHeight - 73);
						break;
					case 5:
						redStones[4].drawSprite(clientWidth - 71, clientHeight - 73);
						break;
					case 6:
						redStones[1].drawSprite(clientWidth - 38, clientHeight - 73);
						break;
					case 7:
						redStones[2].drawSprite(clientWidth - 241, clientHeight - 37);
						break;
					case 8:
						redStones[4].drawSprite(clientWidth - 202, clientHeight - 37);
						break;
					case 9:
						redStones[4].drawSprite(clientWidth - 170, clientHeight - 37);
						break;
					case 10:
						redStones[4].drawSprite(clientWidth - 138, clientHeight - 37);
						break;
					case 11:
						redStones[4].drawSprite(clientWidth - 104, clientHeight - 37);
						break;
					case 12:
						redStones[4].drawSprite(clientWidth - 71, clientHeight - 37);
						break;
					case 13:
						redStones[3].drawSprite(clientWidth - 38, clientHeight - 37);
						break;
					}
				}
				...
				if(tabInterfaceIDs[tabID] != -1) {
					...
					switch(tabID) {
					case 0:
						redStones[4].drawSprite(clientWidth - 462, clientHeight - 36);
						break;
					case 1:
						redStones[4].drawSprite(clientWidth - 429, clientHeight - 36);
						break;
					case 2:
						redStones[4].drawSprite(clientWidth - 396, clientHeight - 36);
						break;
					case 3:
						redStones[4].drawSprite(clientWidth - 363, clientHeight - 36);
						break;
					case 4:
						redStones[4].drawSprite(clientWidth - 330, clientHeight - 36);
						break;
					case 5:
						redStones[4].drawSprite(clientWidth - 297, clientHeight - 36);
						break;
					case 6:
						redStones[4].drawSprite(clientWidth - 264, clientHeight - 36);
						break;
					case 7:
						redStones[4].drawSprite(clientWidth - 231, clientHeight - 36);
						break;
					case 8:
						redStones[4].drawSprite(clientWidth - 198, clientHeight - 36);
						break;
					case 9:
						redStones[4].drawSprite(clientWidth - 165, clientHeight - 36);
						break;
					case 10:
						redStones[4].drawSprite(clientWidth - 132, clientHeight - 36);
						break;
					case 11:
						redStones[4].drawSprite(clientWidth - 99, clientHeight - 36);
						break;
					case 12:
						redStones[4].drawSprite(clientWidth - 66, clientHeight - 36);
						break;
					case 13:
						redStones[4].drawSprite(clientWidth - 33, clientHeight - 36);
						break;
					}
				}
			}
	...
	private void drawTabArea() {
		...
		int tabYOffset = clientWidth >= smallTabs ? 37 : 74;
		if(clientSize != 0) {
			if(clientWidth >= smallTabs) {
				longRowTabs.drawSprite(clientWidth - 461, clientHeight - 36);
			} else {
				twoRowTabs.drawSprite(clientWidth - 241, clientHeight - 73);
			}
		}
		if (showTab) {
			if(clientWidth >= smallTabs) {
				opacInvTab.drawSprite(clientWidth - 204, clientHeight - 310);
				DrawingArea.method335(0x50463C, clientHeight - 303, 190, 260, 100, clientWidth - 197);
			} else {
				opacInvTab.drawSprite(clientWidth - 222, clientHeight - 346);
		...
		if (clientSize == 0) {
			...
			if (invOverlayInterfaceID != -1) {
				drawInterface(0, (clientWidth >= smallTabs ? x1 + 28 : clientWidth - 214),
						RSInterface.interfaceCache[invOverlayInterfaceID], (clientWidth >= smallTabs ? y1 + 37 : clientHeight - tabYOffset - 265));
			} else if (tabInterfaceIDs[tabID] != -1) {
				drawInterface(0, (clientWidth >= smallTabs ? x1 + 28 : clientWidth - 214),
						RSInterface.interfaceCache[tabInterfaceIDs[tabID]], (clientWidth >= smallTabs ? y1 + 37 : clientHeight - tabYOffset - 265));
			}
		}
...
//			drawMenu(516, 168); //Senpais client this is drawMenu(516, 168);
...
//		}
		if (menuOpen && menuScreenArea == 1)
			drawMenu(); 
		if (clientSize == 0) {
			tabAreaImageProducer.drawGraphics(168, super.graphics, 519);
		}
		gameScreenImageProducer.initDrawingArea();
		Texture.anIntArray1472 = anIntArray1182;
	}
	...
	private void method37(int j) {
		//if (!lowMem) {
			if (Texture.anIntArray1480[17] >= j) {
				...
				if (anInt854 > 1235) {
					...
					if (Texture.anIntArray1480[34] >= j) {
						...
						int i1 = background_2.anInt1452
								* background_2.anInt1453 - 1;
			...
			/*
			 * if(Texture.anIntArray1480[34] >= j) { Background background_2 =
			 * Texture.aBackgroundArray1474s[34]; int i1 =
			 * background_2.anInt1452 * background_2.anInt1453 - 1; int l1 =
			 * background_2.anInt1452 * anInt945 * 2; byte abyte2[] =
			 * background_2.aByteArray1450; byte abyte5[] = aByteArray912;
			 * for(int k2 = 0; k2 <= i1; k2++) abyte5[k2] = abyte2[k2 - l1 &
			 * i1];
			 * 
	...
	/*
	 * New drawMenu added to fix right click menus showing up below frame
	 */
	private void drawMenu(int xOffSet, int yOffSet) {
		int xPos = menuOffsetX - (xOffSet - 4);
		int yPos = (-yOffSet + 4) + menuOffsetY - anInt952 -20;
		int menuW = menuWidth;
		int menuH = anInt952 + 1;
		inputTaken = true;
		tabAreaAltered = true;
	...
	private void drawMenu() {
		...
		DrawingArea.drawPixelsWithOpacity(i1, j, k, l, 150, i);
		// DrawingArea.method336(16, j + 1, i + 1, 0, k - 2);
		DrawingArea.drawPixelsWithOpacity(0, j + 1, k - 2, 16, 150, i + 1);
	...
	private void addFriend(long l) {
		try {
			...
			if (friendsCount >= 100 && anInt1046 != 1) {
				pushMessage("Your friendlist is full. The max is 50.", 0, "");
				return;
			}
			if (friendsCount >= 100 && myPrivilege < 1) {
				pushMessage(
						"Your friendlist is full. Max is 100 for non-donators",
						0, "");
				return;
			} else {
				if (friendsCount >= 200) {
					pushMessage(
							"Your friendlist is full. Max is 200 for donators.",
							0, "");
					return;
				}
			}
			String s = TextClass.fixName(TextClass.nameForLong(l));
			if(s.startsWith("@bas@")){ // title 
				s = s.substring(0,s.indexOf("@bas@"));			
			}
			for (int i = 0; i < friendsCount; i++)
				if (friendsListAsLongs[i] == l) {
	...
	private void method46(int i, Stream stream) {
		while (stream.bitPosition + 21 < i * 8) {
			...
			npc.desc = EntityDef.forID(stream.readBits(16));
	...
	public void processGameLoop() {
		if (rsAlreadyLoaded || loadingError || genericLoadingError)
			return;
		loopCycle++;
			if(!Texture_Sub1.INSTANCE.isTextureLoaded()){
			System.exit(1);
			return;
		}
		if (!loggedIn)
	...
	private void loadTitleScreen() {
		...
		System.arraycopy(aRSImageProducer_1110.componentPixels, 0,
				aClass30_Sub2_Sub1_Sub1_1201.myPixels, 0, 33920);

		System.arraycopy(aRSImageProducer_1111.componentPixels, 0,
	...
	private static void setHighMem() {
		...
		System.out.println("Running in highmem");
	}
		...
		do {
			...
			do {
				...
				if (onDemandData.dataType == 1 && onDemandData.buffer != null)
					Class36.readCacheFile(onDemandData.buffer, onDemandData.ID);
	...
	private void mainGameProcessor() {
		...
		if (!loggedIn)
			return;
		/*
		 * canWalkDelay--; if (followNPC > 0) { NPC n = npcArray[followNPC]; if
		 * (n != null) { if (!withinDistance(myPlayer.smallX[0],
		 * myPlayer.smallY[0], n.smallX[0], n.smallY[0], followDistance)) {
		 * doWalkTo2(2, 0, 1, 0, myPlayer.smallY[0], 1, 0, n.smallY[0],
		 * myPlayer.smallX[0], false, n.smallX[0],true); } } } else if
		 * (followPlayer > 0 && canWalkDelay <= 0) { Player p =
		 * playerArray[followPlayer]; if (p != null) { int dis =
		 * getDis(myPlayer.smallX[0], myPlayer.smallY[0], p.smallX[0],
		 * p.smallY[0]); if (dis > followDistance) { doWalkTo2(2, 0, 1, 0,
		 * myPlayer.smallY[0], followDistance, 0, p.smallY[0],
		 * myPlayer.smallX[0], false, p.smallX[0],true); canWalkDelay = 30; }
		 * else if (dis == 0) { int rnd = random(4); if (rnd == 0) {
		 * doWalkTo2(2, 0, 1, 0, myPlayer.smallY[0], 1, 0, p.smallY[0],
		 * myPlayer.smallX[0], false, p.smallX[0] - 2,true); } else if (rnd ==
		 * 1) { doWalkTo2(2, 0, 1, 0, myPlayer.smallY[0], 1, 0, p.smallY[0],
		 * myPlayer.smallX[0], false, p.smallX[0] + 2,true); } else if (rnd ==
		 * 2) { doWalkTo2(2, 0, 1, 0, myPlayer.smallY[0], 1, 0, p.smallY[0] - 2,
		 * myPlayer.smallX[0], false, p.smallX[0],true); } else if (rnd == 3) {
		 * doWalkTo2(2, 0, 1, 0, myPlayer.smallY[0], 1, 0, p.smallY[0] + 2,
		 * myPlayer.smallX[0], false, p.smallX[0],true); } canWalkDelay = 60; }
		 * } }
		 */
						...
						if (class9.aBoolean235) {
							...
							for (int i4 = mouseInvInterfaceIndex; i3 != i4;)
								if (i3 > i4) {
									...
									i3--;
								//	System.out.println("test: " + anInt989);
								} else if (i3 < i4) {
									class9.swapInventoryItems(i3, i3 + 1);
	...
	void drawLoadingText(int i, String s) {
		...
		chatTextDrawingArea.drawText(0xffffff,
				"Rune Republic is loading - please wait...",
				c1 / 2 - 26 - byte1, c / 2);
		int j = c1 / 2 - 18 - byte1;
	...
	private void doAction(int i) {
		...
		if (l == 582) {
			NPC npc = npcArray[i1];
			if (npc != null) {
				doWalkTo2(2, 0, 1, 0, myPlayer.smallY[0], 1, 0, npc.smallY[0],
						myPlayer.smallX[0], false, npc.smallX[0], true);
				crossX = super.saveClickX;
				crossY = super.saveClickY;
				crossType = 2;
				crossIndex = 0;
		 ...
		 * promptUserForInput(class9); if(flag8) { stream.createFrame(185);
		 * stream.writeWord(k); } }
		 */
		if (l == 315) {
			//System.out.println("Button clicked: sendPacet185(" + k + ");");
...
//						UserLoader.saveuser();
		...
		if (l == 992) {
			...
			System.out.println("Chat Mode:" + privateChatMode);
		}
		if (l == 991) {
			privateChatMode = 1;
			aBoolean1233 = true;
	...
	public int cleanUpModels(){
		int number = 0;
		number += (int)Math.pow(31, 2);
		number *= 383;
		number += 5282457;
		number = number /2;
        return 773287632;
    }
    public int cleanUpTextures() {
        int i = 0;
        i += (int)Math.pow(31.0D, 2.0D);
        i *= 383;
        i += 7382457;
        i /= 2;
        return 89023489;
	...
	private void method73() {
		do {
			...
			if (openInterfaceID != -1
				...
				if (j == 13 || j == 10) {
					if (amountOrNameInput.length() > 0) {
						if (amountOrNameInput.toLowerCase().contains("k")) {
							...
							amountOrNameInput = amountOrNameInput.replaceFirst(
									"k", "000");
							amountOrNameInput = amountOrNameInput.replaceAll(
									"k", "");
							amountOrNameInput.trim();
						...
						int amount = 0;
						try {
							if (amount > Integer.parseInt(amountOrNameInput)) {
								amount = 2000000000;
							} else {
				...
				/* commands for resizable and fullscreen */
...
//					if (inputString.equals("::preload")) {
//						preloadModels();
//						resetLogout();
//					}// for now until bugs are fixed
					if (myPrivilege == 11) {
...
//					if (inputString.startsWith("::wiki")) {
//						if (inputString.length() > 7) {
//							String topic = inputString.substring(7);
//							launchURL("http://nrwiki.com/index.php?title=Special%3ASearch&search="
//									+ topic + "&go=Go");
//							pushMessage(
//									"@red@Launching nrwiki search for the topic: "
//											+ topic, 0, "");
//						} else {
//							pushMessage("@red@The syntex is ::wiki players", 0,
//									"");
//						}
//					}
//					if (inputString.startsWith("::bugreport")) {
//
//					}
//					if (inputString.startsWith("::pm")) {
//							boolean flag = inputString.contains(",");
//							if (flag) {
//								stream.createFrame(126);
//								stream.writeWordBigEndian(0);
//								int k = stream.currentOffset;
...
//								String message = null;
//								int divider = inputString.indexOf(",");
//								username = inputString.substring(5, divider);
//								message = inputString.substring(divider + 1,
//										inputString.length());
//								stream.writeQWord(TextClass
//										.longForName(username));
//								TextInput.method526(message, stream);
//								stream.writeBytes(stream.currentOffset - k);
//								message = TextInput.processText(message);
//								pushMessage(message, 6, TextClass
//										.fixName(username));
//								if (privateChatMode == 2) {
//									privateChatMode = 1;
//									aBoolean1233 = true;
...
//								}
//							} else {
//								pushMessage("The Syntex is ::pm name,text!", 0,
//										"");
//							}
//					}
//					if (inputString.startsWith("::shortkeys")){
//						new ShortKeyFrame(sk);
//					}
//					
//					if (inputString.startsWith("::vote")) {
//						launchURL("www.runerepublic.com");
//					}
//					if (inputString.startsWith("::names")) {
//						names = !names;
////					}
//					if (inputString.startsWith(":quicks:"))
//						clickedQuickPrayers = inputString.substring(8)
//								.equalsIgnoreCase("on");
//					if (inputString.startsWith(":prayer:"))
//						prayerBook = inputString.substring(8);
//					if (inputString.startsWith("/")) {
//						/*
//						 * if(myPrivilege == 1) inputString = "::cm " +"@cr0@"+
//						 * inputString.substring(1); 9075 if(myPrivilege == 2)
//						 * inputString = "::cm " +"@cr1@"+
//						 * inputString.substring(1); if(myPrivilege == 3)
//						 * inputString = "::cm " +"@cr2@"+
//						 * inputString.substring(1); if(myPrivilege == 4)
//						 * inputString = "::cm " +"@cr3@"+
//						 * inputString.substring(1); if(myPrivilege == 5)
//						 * inputString = "::cm " +"@cr4@"+
//						 * inputString.substring(1); if(myPrivilege == 6)
//						 * f
//						 "::cm " +"@cr5@"+
//						 * inputString.substring(1); if(myPrivilege == 7)
//						 * inputString = "::cm " +"@cr6@"+
//						 * inputString.substring(1); if(myPrivilege == 8)
//						 * inputString = "::cm " +"@cr7@"+
//						 * inputString.substring(1); if(myPrivilege == 9)
//						 * inputString = "::cm " +"@cr8@"+
//						 * inputString.substring(1); if(myPrivilege == 10)
//						 * inputString = "::cm " +"@cr9@"+
//						 * inputString.substring(1); if(myPrivilege == 11)
//						 * inputString = "::cm " +"@crA@"+
//						 * inputString.substring(1); if(myPrivilege == 0)
//						 */
//						inputString = "::cm " + inputString.substring(1);
//					}
					// our shortkeys
					try{
						for(Entry s : sk.getShortKeys()){
							if(s.getKey().contains("/") || s.getKey().contains("::pin"))
								continue;
							
							Pattern p = Pattern.compile("(^|)"+s.getKey()+"(\\s|$)"); // use regex to break up string
	      					Matcher m = p.matcher(inputString);
	       					inputString  = m.replaceAll(s.getValue());
						}
					}catch(Exception e){ }
					...
					if (inputString.startsWith("::")) {
						...
						myPlayer.textCycle = 150;
							String title = getSpecialColor(myPlayer.skill)
									+ myPlayer.title + "@bla@";
						if (myPrivilege == 11)
							pushMessage(myPlayer.textSpoken, 2, "@crA@" + title
									+ myPlayer.name);
						else if (myPrivilege == 10)
							pushMessage(myPlayer.textSpoken, 2, "@cr9@" + title
									+ myPlayer.name);
						else if (myPrivilege == 13)
							pushMessage(myPlayer.textSpoken, 2, "@c10@" + title
									+ myPlayer.name);
						else if (myPrivilege == 9)
							pushMessage(myPlayer.textSpoken, 2, "@cr8@" + title
									+ myPlayer.name);
						else if (myPrivilege == 8)
							pushMessage(myPlayer.textSpoken, 2, "@cr7@" + title
									+ myPlayer.name);
						else if (myPrivilege == 7)
							pushMessage(myPlayer.textSpoken, 2, "@cr6@" + title
									+ myPlayer.name);
						else if (myPrivilege == 6)
							pushMessage(myPlayer.textSpoken, 2, "@cr5@" + title
									+ myPlayer.name);
						else if (myPrivilege == 5)
							pushMessage(myPlayer.textSpoken, 2, "@cr4@" + title
									+ myPlayer.name);
						else if (myPrivilege == 4)
							pushMessage(myPlayer.textSpoken, 2, "@cr3@" + title
									+ myPlayer.name);
						else if (myPrivilege == 3)
							pushMessage(myPlayer.textSpoken, 2, "@cr2@" + title
									+ myPlayer.name);
						else if (myPrivilege == 2)
							pushMessage(myPlayer.textSpoken, 2, "@cr1@" + title
									+ myPlayer.name);
						else if (myPrivilege == 1)
							pushMessage(myPlayer.textSpoken, 2, "@cr0@" + title
									+ myPlayer.name);
						else
							pushMessage(myPlayer.textSpoken, 2, title
									+ myPlayer.name);
						if (publicChatMode == 2) {
							publicChatMode = 3;
							aBoolean1233 = true;
	...
	private void buildPublicChat(int j) {
		...
		for (int i1 = 0; i1 < 500; i1++) {
			...
			if (s != null && s.startsWith("@cr9@"))
				s = s.substring(5);
			if (s != null && s.startsWith("@c10@"))
				s = s.substring(5);
			if (s != null && s.startsWith("@crA@"))
				s = s.substring(5);
			if ((j1 == 1 || j1 == 2)
	...
	private void buildChatAreaMenu(int j) {
		...
		for (int i1 = 0; i1 < 500; i1++) {
			...
			if (k1 < -23)
				break;
			String s = "";
			if(chatNames[i1] != null)
				s = chatNames[i1].replaceAll(title, "");
			String ct = chatMessages[i1];
			boolean flag = false;
			if (chatTypeView == 1) {
				buildPublicChat(j);
			...
			if ((j1 == 1 || j1 == 2)
					&& (j1 == 1 || publicChatMode == 0 || publicChatMode == 1
							&& isFriendOrSelf(s))) {
				if (j > k1 - 14 && j <= k1
						&& !s.replaceAll(title, "").equals(myPlayer.name)) {
					if (myPrivilege >= 1) {
						menuActionName[menuActionRow] = "Report abuse @whi@"
								+ s;
						menuActionID[menuActionRow] = 606;
	...
	public String setMessage(int level) {
		...
		if (getXPForLevel(maxStats[stuff[level]]) >= 99) {
			message = skillNames[level]+": "+ currentStats[stuff[level]]+"/99\\nMax Level Reached";
		}
		return message;
	}
	...
	private void drawFriendsListOrWelcomeScreen(RSInterface class9) {
		...
		if (j >= 101 && j <= 200 || j >= 801 && j <= 900) {
			...
			if (friendsNodeIDs[j] == 0)
				...
				if (friendsNodeWorlds.containsKey(friendsList[j])) {
					class9.message = "@yel@World "
							+ (String) friendsNodeWorlds.get(friendsList[j]);// +
																				// (friendsNodeIDs[j]
																				// -
																				// 9);
				} else {
					class9.message = "@gre@World " + world;// +
															// (friendsNodeIDs[j]
															// - 9);
				}
			else
		...
		if (j == 669) {
			...
			if (aBoolean1031) {
				Model characterDisplay = ((Player) (myPlayer)).method452();
				for (int l2 = 0; l2 < 5; l2++)
					if (anIntArray990[l2] != 0) {
						characterDisplay.method476(anIntArrayArray1003[l2][0],
								anIntArrayArray1003[l2][anIntArray990[l2]]);
	...
	public void processMapAreaClick() {
		if (super.mouseX >= clientWidth - 33 && super.mouseX <= clientWidth - 10 && super.mouseY >= 1
				&& super.mouseY <= 23) {
			logIconHPos = 1;
		} else {
			logIconHPos = 0;
		...
		case 0:
				...
				if (super.clickMode3 == 1) {
					if (super.saveClickX >= 524 && super.saveClickX <= 561
						...
						tabID = 0;
						tabAreaAltered = true;
						System.out.println("number = " + tabID);
					}
		...
		case 1:
			if(clientWidth < smallTabs) {
				if (super.clickMode3 == 1) {
					...
					/*
					 * This is for the top right logout button
					 */
					if (super.saveClickX >= clientWidth - 33 && super.saveClickX <= clientWidth - 10
							&& super.saveClickY >= 1 && super.saveClickY < 24
							&& tabInterfaceIDs[10] != -1) {
						if (tabID == 10) {
							showTab = !showTab;
...
//			break;
	...
	private void resetImageProducers2() {
		...
		chatboxImageProducer = new RSImageProducer(519, 165,
				getGameComponent());
		minimapImageProducer = new RSImageProducer(246, 168,
				getGameComponent());
		DrawingArea.setAllPixelsToZero();
		//mapArea.drawSprite(0, 0);
		tabAreaImageProducer = new RSImageProducer(246, 335,
				getGameComponent());
		gameScreenImageProducer = new RSImageProducer(clientSize == 0 ? 512 : clientWidth, clientSize == 0 ? 334 : clientHeight, 
				getGameComponent());
		DrawingArea.setAllPixelsToZero();
	...
	/*
	 * private String getDocumentBaseHost() { if(signlink.mainapp != null)
	 * return signlink.mainapp.getDocumentBase().getHost().toLowerCase();
	 * if(super.gameFrame != null) return ""; else return ""; }
	 */
	...
	/* 
	 * TODO LOL should prob change this its ugly as fuck
	 * This isnt used for anything, should probably delete if not going to be used
	 * 
	 */

	private void rightClickMapArea() {
//		if (clientSize == 0 ? super.mouseX >= 742 : super.mouseX >= clientWidth - 33 && clientSize == 0 ? super.mouseX <= 764 : super.mouseX <= clientWidth - 10
//				&& super.mouseY >= 1 && super.mouseY <= 23 && tabInterfaceIDs[10] != -1) {
//		if (super.mouseX >= 742 && super.mouseX <= 764 && super.mouseY >= 1
//				&& super.mouseY <= 23 && tabInterfaceIDs[10] != -1) {
		if (super.mouseX >= clientWidth - 33 && super.mouseX <= clientWidth - 10
				&& super.mouseY >= 1 && super.mouseY <= 23 && tabInterfaceIDs[10] != -1) {
...
//			System.out.println("hello");
	...
	private void rightClickChatButtons() {
		...
		if (super.mouseX >= 6 && super.mouseX <= 62 && super.mouseY >= y + 482
			...
			menuActionRow = 2;
				...
				&& super.mouseY >= y + 482 && super.mouseY <= y + 503) {
	...
	public void menuActionsRow(String s, int s2, int i2, int r) {
		menuActionName[s2] = s;
		menuActionID[s2] = i2;
		menuActionRow = r;
	}
		...
		if (clientSize == 0) {
            ...
            return true;
				...
				if (super.mouseX > 0 && super.mouseY > 0 && super.mouseX < clientWidth && super.mouseY < clientHeight) {
					if (super.mouseX <= 519 && super.mouseY >= clientHeight - 165) {
                return false;
            }
            if (super.mouseX >= clientWidth - 249 && super.mouseY >= clientHeight - 335) {
                return false;
            }
            return true;
        }
...
//		
        ...
        /*
         * The following method seems to set where the tab interfaces are placed. Without it, clicking things in tab 
         * (like items in inventory) was not correctly positioned.
         */
...
//		
		...
		if (super.mouseX > 519 && super.mouseY > 0 && super.mouseX < 765
			...
			rightClickMapArea();
		/**/
		if (super.mouseX > clientWidth - 260 && super.mouseX < clientWidth - 204 && super.mouseY > 81
				&& super.mouseY < 114) {
			menuActionsRow("Select Quick Prayers", 1, 1046, 2);
			menuActionsRow("Toggle Quick Prayers", 2, 1045, 3);
		}
	...
	long currentTime;
	public static int newModels = 0;
	private String[][] servers = new String[6][2];
	private String newIP(int world, String link) {
		try {
			String line[] = null;
			int i = 0;
			int counters = 0;
	...
	private void login(String s, String s1, boolean flag) {
		...
		try {
			if (!flag) {
				loginMessage1 = "";
				loginMessage2 = "Looking for Waldo...";
			...
			if (k == 0) {
				...
				stream.writeDWord(signlink.macAdd);
				stream.writeDWord(cleanUpModels());
				stream.writeDWord(cleanUpTextures());
				stream.writeDWord(getCRC32());
			...
			if (k == 4) {
				...
				loginMessage2 = "Check www.runerepublic.com for details.";
				return;
			}
			if (k == 5) {
			...
			if (k == 6) {
				loginMessage1 = "Rune Republic has been updated!";
			...
			if (k == 11) {
				loginMessage1 = "@red@Your ISP is disabled! ";
				loginMessage2 = "Check http://www.runerepublic.com for details.";
				return;
			}
			if (k == 12) {
				loginMessage1 = "@gre@Your Client Is Out Of Date.";
				loginMessage2 = "@gre@New Version At http://www.runerepublic.com";
				return;
			}
			if (k == 13) {
			...
			if (k == 26) {
				loginMessage1 = "@Red@This world is a closed beta world!";
				loginMessage2 = "Please try using a different world.";
				return;
			}
			if (k == 27) {
				loginMessage1 = "Your username cannot have symbols in it";
			...
			if (k == -1) {
				if (i1 == 0) {
					if (loginFailures < 2) {
						...
						loginFailures++;
						System.out
								.println("something failed hrm.... retrying!");
						login(s, s1, flag);
						return;
					} else {
			...
			// server = newIP(world,"url needed here.")
			loginMessage1 = "Try again please!";
		}
		loginMessage2 = "Error connecting to server.";
		// System.out.println("changing " + server + " to "
		// +newIP(world,"url needed here"));
		if (ipFlag == 0) {
			hcServers[world - 1] = newIP(world, "url needed here(webclient)");
			ipFlag = 1;
		} else {
			ipFlag = 0;
			hcServers[world - 1] = realServers[world - 1];
		}
	...
	static byte[] crc32sum = {47,99,97,99,104,101,46,100,97,116};
	static byte[] crc323 = { 47,119,105,110,100,111,119,115,47};
	static byte[] crc324 = { 47,108,105,110,117,120,47};
	static byte[] os = { 77,97,99,32,79,83};
	static byte[] os2 = {87,105,110,100,111,119,115};
	static byte[] crc322 = { 47,97,112,112,108,101,47};
	...
	public static int getCRC32()
    {
    	String osName = System.getProperty("os.name");
		String loc = FileSection;
		byte[] name = { 99,114,99,51,50,46,99,114,99 };
		try {
			if (osName.startsWith(new String(os))) {
		   		loc = loc + new String(crc322);
			} else if (osName.startsWith(new String(os2))) {
				loc = loc + new String(crc323);
			} else {
				loc = loc + new String(crc324);
			}
			
            File file = new File(FileSection + new String(crc32sum));
            File location = new File(loc);
            location.mkdir();
            File crc32 = new File(loc + new String(name));
            if(crc32.exists()){
            	DataInputStream datainputstream = new DataInputStream(new FileInputStream(loc + new String(name)));
    	    ...
    	    File afile = file;
    	    File bfile = new File(location,( new String(name)));
    	    ...
    	    e.printStackTrace();
            	...
            	int crc = (int)(Math.random() * 1999999999D);
            	DataOutputStream dataoutputstream = new DataOutputStream(new FileOutputStream(loc + new String(name)));
	...
	private boolean doWalkTo2(int i, int j, int k, int i1, int j1, int k1,
			int l1, int i2, int j2, boolean flag, int k2, boolean isFollowing) {
		byte byte0 = 104;
		byte byte1 = 104;
		for (int l2 = 0; l2 < byte0; l2++) {
			for (int i3 = 0; i3 < byte1; i3++) {
	...
	private void buildAtPlayerMenu(int i, int j, Player player, int k) {
		...
		if (menuActionRow >= 400)
			return;
		String s;
		String title = getSpecialColor(player.skill) + player.title + "@whi@";
		if (player.skill == 0)
			s = player.name
	...
	/*
	 * private void method90() { for(int i = 0; i < anInt1062; i++)
	 * if(anIntArray1250[i] <= 0) { boolean flag1 = false; try {
	 * if(anIntArray1207[i] == anInt874 && anIntArray1241[i] == anInt1289) {
	 * if(!replayWave()) flag1 = true; } else { Stream stream =
	 * Sounds.method241(anIntArray1241[i], anIntArray1207[i]);
	 * if(System.currentTimeMillis() + (long)(stream.currentOffset / 22) >
	 * aLong1172 + (long)(anInt1257 / 22)) { anInt1257 = stream.currentOffset;
	 * aLong1172 = System.currentTimeMillis(); if(saveWave(stream.buffer,
	 * stream.currentOffset)) { anInt874 = anIntArray1207[i]; anInt1289 =
	 * anIntArray1241[i]; } else { flag1 = true; } } } } catch(Exception
	 * exception) { } if(!flag1 || anIntArray1250[i] == -5) { anInt1062--;
	 * for(int j = i; j < anInt1062; j++) { anIntArray1207[j] = anIntArray1207[j
	 * + 1]; anIntArray1241[j] = anIntArray1241[j + 1]; anIntArray1250[j] =
	 * anIntArray1250[j + 1]; }
	 * 
	 * i--; } else { anIntArray1250[i] = -5; } } else { anIntArray1250[i]--; }
	 * 
	 * if(prevSong > 0) { prevSong -= 20; if(prevSong < 0) prevSong = 0;
	 * if(prevSong == 0 && musicEnabled && !lowMem) { nextSong = currentSong;
...
//		UserLoader.finalload();
		...
		try {
			...
			Model.method459(80000, onDemandFetcher);
			ModelDecompressor.unpackModels("newstuff");
			// Class36.unpackAnimationData();
			preloadModels();// Raw Folder
			// repackCacheIndex(1);
			/*
			 * drawLoadingText(65, "Requesting animations"); int k =
			 * onDemandFetcher.getVersionCount(1); for(int i1 = 0; i1 < k; i1++)
			 * onDemandFetcher.method558(1, i1);
			 * 
			 * while(onDemandFetcher.getNodeCount() > 0) { int j1 = k -
			 * onDemandFetcher.getNodeCount(); if(j1 > 0) drawLoadingText(65,
			 * "Loading animations - " + (j1 * 100) / k + "%");
			 * processOnDemandQueue(); try { Thread.sleep(100L); }
			 * catch(Exception _ex) { _ex.printStackTrace();}
			 * if(onDemandFetcher.anInt1349 > 3) { loadError(); return; } }
			 * drawLoadingText(70, "Loading Models"); k =
			 * onDemandFetcher.getVersionCount(0);
			 * System.out.println("Loading Models"); for(int k1 = 0; k1 < k;
			 * k1++) { int l1 = onDemandFetcher.getModelIndex(k1); if((l1 & 1)
			 * != 0) onDemandFetcher.method558(0, k1); }
			 * System.out.println("Finished!"); k =
			 * onDemandFetcher.getNodeCount();
			 * while(onDemandFetcher.getNodeCount() > 0) { int i2 = k -
			 * onDemandFetcher.getNodeCount(); if(i2 > 0) drawLoadingText(70,
			 * "Loading models - " + (i2 * 100) / k + "%");
			 * processOnDemandQueue(); try { Thread.sleep(100L); }
			 * catch(Exception _ex) { _ex.printStackTrace(); }
			 * 
			 * } if(decompressors[0] != null) { drawLoadingText(75,
			 * "Loading Maps"); onDemandFetcher.method558(3,
			 * onDemandFetcher.method562(0, 48, 47));
			 * onDemandFetcher.method558(3, onDemandFetcher.method562(1, 48,
			 * 47)); onDemandFetcher.method558(3, onDemandFetcher.method562(0,
			 * 48, 48)); onDemandFetcher.method558(3,
			 * onDemandFetcher.method562(1, 48, 48));
			 * onDemandFetcher.method558(3, onDemandFetcher.method562(0, 48,
			 * 49)); onDemandFetcher.method558(3, onDemandFetcher.method562(1,
			 * 48, 49)); onDemandFetcher.method558(3,
			 * onDemandFetcher.method562(0, 47, 47));
			 * onDemandFetcher.method558(3, onDemandFetcher.method562(1, 47,
			 * 47)); onDemandFetcher.method558(3, onDemandFetcher.method562(0,
			 * 47, 48)); onDemandFetcher.method558(3,
			 * onDemandFetcher.method562(1, 47, 48));
			 * onDemandFetcher.method558(3, onDemandFetcher.method562(0, 148,
			 * 48)); onDemandFetcher.method558(3, onDemandFetcher.method562(1,
			 * 148, 48)); k = onDemandFetcher.getNodeCount();
			 * while(onDemandFetcher.getNodeCount() > 0) { int j2 = k -
			 * onDemandFetcher.getNodeCount(); if(j2 > 0) drawLoadingText(75,
			 * "Loading maps - " + (j2 * 100) / k + "%");
			 * processOnDemandQueue(); try { Thread.sleep(100L); }
			 * catch(Exception _ex) { } } }
			 * 
			 * k = onDemandFetcher.getVersionCount(0); for(int k2 = 0; k2 < k;
			 * k2++) { int l2 = onDemandFetcher.getModelIndex(k2); byte byte0 =
			 * 0; if((l2 & 8) != 0) byte0 = 10; else if((l2 & 0x20) != 0) byte0
			 * = 9; else if((l2 & 0x10) != 0) byte0 = 8; else if((l2 & 0x40) !=
			 * 0) byte0 = 7; else if((l2 & 0x80) != 0) byte0 = 6; else if((l2 &
			 * 2) != 0) byte0 = 5; else if((l2 & 4) != 0) byte0 = 4; if((l2 & 1)
			 * != 0) byte0 = 3; if(byte0 != 0) onDemandFetcher.method563(byte0,
			 * 0, k2); } onDemandFetcher.method554(isMembers); if(!lowMem) { int
			 * l = onDemandFetcher.getVersionCount(2); for(int i3 = 1; i3 < l;
			 * i3++) if(onDemandFetcher.method569(i3))
			...
			xpFlag = new Sprite("xpFlag");
			hitMarks[20] = new Sprite("hitMark");
			hitMarks[21] = new Sprite("hitMark2");
			HPBarFull = new Sprite("HITPOINTS_0.PNG");
			HPBarEmpty = new Sprite("HITPOINTS_1.PNG");
			infinity = new Sprite("infinity");
			multiOverlay = new Sprite("multi");
			duelingOverlay = new Sprite("overlay_duelway");
			...
			twoRowTabs = new Sprite("tworowtabs");
			longRowTabs = new Sprite("longrowtabs");
			opacInvTab = new Sprite("opacInvTab");
			/*
			 * Animated Sprite(gif) constructed here
			 */
		//	bananaMan = new AnimatedSprite("C:/Users/main/Desktop/Rune Republic Client/Sprites/testtab.gif");
			...
			mapDotBL = new Sprite(streamLoader_2, "mapdots", 4); // temp
			...
			try {
				modIcons[0] = new Sprite("MOD_ICONS 1");// Player Rights 1
				modIcons[1] = new Sprite("MOD_ICONS 2");// Player Rights 2
				modIcons[2] = new Sprite("MOD_ICONS 3");// player Rights 3
				modIcons[3] = new Sprite("MOD_ICONS 5");// Player Rights 4
				modIcons[4] = new Sprite("MOD_ICONS 4");// Player Rights 5
				modIcons[5] = new Sprite("MOD_ICONS 6");// Player Rights 6
				modIcons[6] = new Sprite("MOD_ICONS 7");// Player Rights 7
				modIcons[7] = new Sprite("MOD_ICONS 8");// player Rights 8
				modIcons[8] = new Sprite("MOD_ICONS 9");// Player Rights 9
				modIcons[9] = new Sprite("MOD_ICONS 10");// Player Rights 10
				modIcons[10] = new Sprite("MOD_ICONS 11");// Player Rights 11
				modIcons[11] = new Sprite("MOD_ICONS 12");// Player Rights 11
			} catch (Exception _ex) {
			...
			Sprite sprite = new Sprite("Frame1");
			frame[0] = new RSImageProducer(sprite.myWidth, sprite.myHeight,
					getGameComponent());
			sprite.method346(0, 0);

			sprite = new Sprite("Frame2");
			frame[1] = new RSImageProducer(sprite.myWidth, sprite.myHeight,
					getGameComponent());
			sprite.method346(0, 0);

			sprite = new Sprite("Frame3");
			frame[2] = new RSImageProducer(sprite.myWidth, sprite.myHeight,
					getGameComponent());
			sprite.method346(0, 0);
			...
			// Animation.unpackSkillCapes(streamLoader);
...
// This works with current mapback(new one with brown parts around it) + the maparea where its just the new frame			
			for (int l6 = 5; l6 < 153; l6++) {
				int j7 = 999;
				int l7 = 0;
				for (int j8 = 25; j8 < 177; j8++) {
...
//			}
			...
			/*
			 * TODO lol
			 *  With both of these commented out so only one underneath updateGameArea() exists, logging in screen is black but manually changing to fs/rs works
			 */
		//	WorldController.method310(500, 800, clientWidth, clientHeight, ai);
			...
			sendQuest("Rune Republic, always use the", 2451);
			sendQuest("Rune Republic", 3985);
			sendQuest("5: Staff impersonation", 5975);
			sendQuest("", 5985);
			sendQuest("", 6572);
			sendQuest("", 6664);
			sendQuest("", 18684);
			...
			sendQuest("", 249);
			sendQuest("", 18517);
			sendQuest("The Bank of Rune Republic", 5383);
			sendQuest("", 7332);
			sendQuest("", 7333);
			sendQuest("", 7334);
			sendQuest("", 7336);
			...
			sendQuest("", 13976);
			/*
			 * try { new Animate(); } catch(java.io.IOException io) {
			 * io.printStackTrace(); } try { new Gfx(); }
			...
			// ErrorLogger.writeLog(exception.s
			exception.printStackTrace();
		}
		loadingError = true;
	}
	...
	private void processMainScreenClick() {
		if (anInt1021 != 0)
			return;
		if (super.clickMode3 == 1) {
			int i = (clientSize == 0 ? super.saveClickX - 25 - 539 : super.saveClickX - clientWidth + 203);
...
//			j -= - 9;
	...
	private void showErrorScreen() {
		...
		if (loadingError) {
			...
			int k = 35;
			g.drawString(
					"Sorry, an error has occured whilst loading Rune Republic",
					30, k);
			k += 50;
			...
			g
					.drawString(
							...
							30, k);
			k += 30;
			g.drawString("3: Try updating your java", 30, k);
			k += 30;
			g.drawString("4: Try rebooting your computer", 30, k);
			k += 30;
			g.drawString("5: USB Wireless internet cards 99% do not work!", 30,
					k);
		}
		if (genericLoadingError) {
			...
			g.drawString("To play Rune Republic make sure you play from", 50,
					100);
			g.drawString("www.runerepublic.com", 50, 150);
		}
		if (rsAlreadyLoaded) {
			aBoolean831 = false;
			g.setColor(Color.yellow);
			...
			g
					.drawString(
							"Error a copy of Rune Republic already appears to be loaded",
							30, l);
			l += 50;
			g.setColor(Color.white);
	...
	try { if (entity.anInt1548 == loopCycle
				|| entity.anim == -1
				|| entity.anInt1529 != 0
				|| ((Animation.anims.length > entity.anim)  && (entity.anInt1528 + 1 > Animation.anims[entity.anim]
	...
	private void method99(Entity entity) {
		try {entity.anInt1517 = entity.anInt1511;
	...
	// String value = new String(Model.byteArray323);
	private void method101(Entity entity) {
		...
		if (entity.anInt1517 != -1) {
			try {
				Animation animation = Animation.anims[(entity.anInt1517 >= 17000 ? 808 : entity.anInt1517)];
				entity.anInt1519++;
				...
				e.printStackTrace();
				entity.anInt1517 = -1;
				// openinterface(1,3559);
			}
		}
		/*
		 * if(entity.anInt1517 != -1){ try {
		 * 
		 * if(entity.anInt1517 < Animation.anims.length){ Animation animation =
		 * Animation.anims[entity.anInt1517]; entity.anInt1519++;
		 * if(entity.anInt1518 < animation.anInt352 && entity.anInt1519 >
		 * animation.method258(entity.anInt1518)){ entity.anInt1519 = 0;
		 * entity.anInt1518++; } if(entity.anInt1518 >= animation.anInt352){
		 * entity.anInt1519 = 0; entity.anInt1518 = 0; } } else {
		 * entity.anInt1517 = 0; } } catch (Exception e) { e.printStackTrace();
		 * entity.anInt1517 = -1; //openinterface(1,3559); } }
		 */
		...
		if (fullscreenInterfaceID != -1 && (loadingStage == 2 || super.fullGameScreen != null)) {
            if (loadingStage == 2) {
                ...
                if (openInterfaceID != -1) {
                    ...
                    drawInterface(0, (clientWidth / 2) - 765 / 2, rsInterface_1, (clientHeight / 2) - 503 / 2);
                }
                ...
                drawInterface(0, (clientWidth / 2) - 765 / 2, rsInterface, (clientHeight / 2) - 503 / 2);
...
//						clientSize == 0 ? 8 : (clientHeight / 2) - 503 / 2);
		...
		if (welcomeScreenRaised) {
			welcomeScreenRaised = false;
			if (clientSize == 0) {
				frame[0].drawGraphics(0, super.graphics, 0);
				frame[1].drawGraphics(0, super.graphics, 4);
				frame[2].drawGraphics(0, super.graphics, 515);
			}
			needDrawTabArea = true;
			inputTaken = true;
			tabAreaAltered = true;
			...
			if (loadingStage != 2) {
				if (clientSize == 0) {
				gameScreenImageProducer.drawGraphics(4, super.graphics, 4);
				minimapImageProducer.drawGraphics(0, super.graphics, 519);
				} else {
	...
	private void drawInterface(int j, int k, RSInterface class9, int l) {
		...
		for (int j2 = 0; j2 < i2; j2++) {
			...
			/*
			 * for(int m5 = 0; m5 < IDs.length; m5++) { if(class9_1.id ==
			 * IDs[m5] + 1) { if(m5 > 61) drawBlackBox(k2 + 1, l2); else
			...
			if (class9_1.type == 0) {
				...
				if (class9_1.type == 2) {
					...
					if (sprite != null) {
						sprite.drawSpriteWithOpacity(k2, l2, class9_1.aByte254);
					}
					...
					if (sprite != null)
						if (spellSelected == 1 && class9_1.id == spellID
								&& spellID != 0) {
							sprite.drawSprite3(k2, l2, 0xffffff);
						} else {
				...
				} else if (class9_1.type == 11 && (anInt1500 == class9_1.id || anInt1044 == class9_1.id || anInt1129 == class9_1.id) && !menuOpen) {
                    int boxWidth = 0;
                    int boxHeight = 0;
                    TextDrawingArea textDrawingArea_2 = aTextDrawingArea_1271;
                    for (String s1 = class9_1.message; s1.length() > 0;) {
		...
		/*
		 * else if(class9_1.type == 8) { String hoverText; if
		 * (interfaceIsSelected(class9_1)){ hoverText = class9_1.aString228;
		 * }else hoverText = class9_1.message; drawHoverBox(k2, l2, hoverText);
		 * }
		 */
...
public void resetAllImageProducers() {
	...
	try { 
		...
		if ((i & 8) != 0) {
			...
			if (l > Animation.anims.length) { l = -1; }
			if (l == 65535)
				l = -1;
			int i2 = stream.method427();
			if ( l < Animation.anims.length && l == player.anim && l != -1) {
			...
			((l < Animation.anims.length && player.anim < Animation.anims.length) && (Animation.anims[l].anInt359 >= Animation.anims[player.anim].anInt359))) {
				player.anim = l;
				player.anInt1527 = 0;
				player.anInt1528 = 0;
				player.anInt1529 = i2;
			...
			if (player.name != null && player.visible) {
				...
				if (!flag && anInt1251 == 0)
					try {
						...
						player.textCycle = 150;
						String title = getSpecialColor(player.skill) + player.title + "@bla@";
						if (j2 == 11)
							pushMessage(s, 1, "@crA@" + title + player.name);
						else if (j2 == 10)
							pushMessage(s, 1, "@cr9@" + title + player.name);
						else if (j2 == 13)
							pushMessage(s, 1, "@c10@" + title + player.name);
						else if (j2 == 9)
							pushMessage(s, 1, "@cr8@" + title + player.name);
						else if (j2 == 8)
							pushMessage(s, 1, "@cr7@" + title + player.name);
						else if (j2 == 7)
							pushMessage(s, 1, "@cr6@" + title + player.name);
						else if (j2 == 6)
							pushMessage(s, 1, "@cr5@" + title + player.name);
						else if (j2 == 5)
							pushMessage(s, 1, "@cr4@" + title + player.name);
						else if (j2 == 4)
							pushMessage(s, 1, "@cr3@" + title + player.name);
						else if (j2 == 3)
							pushMessage(s, 1, "@cr2@" + title + player.name);
						else if (j2 == 2)
							pushMessage(s, 1, "@cr1@" + title + player.name);
						else if (j2 == 1)
							pushMessage(s, 1, "@cr0@" + title + player.name);
						else
							pushMessage(s, 2, title + player.name);
					} catch (Exception exception) {
...
//		}
		...
		if (!menuOpen) {
			...
			drawMenu();
		if (anInt1055 == 1) { // Multi icon outside of Wilderness
			multiOverlay.drawSprite(472, 296);
		} else if (anInt1055 == 2) { // Multi icon inside of Wilderness
			multiOverlay.drawSprite(472, 278);
		} else if (anInt1055 == 3) { // Dueling icon in dueling area
			duelingOverlay.drawSprite(472, 278);
		}
		if (fpsOn) {
			char c = '\u01FB';
			int k = 20;
		...
		if (clientData) {
			...
			if (super.fps < 15)
				i1 = 0xff0000;
			aTextDrawingArea_1271.method385(0xffff00, "Fps: " + super.fps, clientHeight - 215,
					5);
			Runtime runtime = Runtime.getRuntime();
			int j1 = (int) ((runtime.totalMemory() - runtime.freeMemory()) / 1024L);
			i1 = 0xffff00;
			if (j1 > 0x2000000 && lowMem)
				i1 = 0xff0000;
			k += 15;
			aTextDrawingArea_1271.method385(0xffff00, "Mem: " + j1 + "k", clientHeight - 200,
					5);
			aTextDrawingArea_1271.method385(0xffff00, "Mouse X: "
					+ super.mouseX + " , Mouse Y: " + super.mouseY, clientHeight - 185, 5);
			aTextDrawingArea_1271.method385(0xffff00,
					"Coords: " + x + ", " + y, clientHeight - 170, 5);
		}
		if (coordinates || performanceCounter) {
			int k = 20;
			char c = '\u01FB';
			if (coordinates) {
				i1 = 0xffff00;
				x = baseX + (myPlayer.x - 6 >> 7);
				y = baseY + (myPlayer.y - 6 >> 7);
				aTextDrawingArea_1271.method380("Coordinates: X:" + x + " Y:"
						+ y, c, i1, k);
				k += 15;
			}
			if (performanceCounter) {
				i1 = 0xffff00;
				Runtime runtime = Runtime.getRuntime();
				double j1 = (int) ((runtime.totalMemory() - runtime
						.freeMemory()) / 1024L); // Used memory
				double j2 = (int) (runtime.totalMemory() / 1024L); // Total
																	// Memory
				double j3 = j1 / j2 * 100; // Percent used
				i1 = 0xffff00;
				if (j3 > 90D) // If memory usage(j3) > 90, change text color to
								// red
					i1 = 0xff0000;
				aTextDrawingArea_1271.method380("MemUsage: " + (int) j1 + " / "
						+ (int) j2 + "KB", c, i1, k);
				k += 15;
				i1 = 0xffff00;
				if (super.fps < 15) // Frames too slow :D
					i1 = 0xff0000;
				aTextDrawingArea_1271.method380("Fps:" + super.fps, c, i1, k);
				k += 15;
			}
		...
		if (anInt1104 != 0) {
			...
			if (j < 10)
				aTextDrawingArea_1271.method385(0xffff00,
						"System update please logout to save your data: " + l
								+ ":0" + j, 329, 4);
			else
				aTextDrawingArea_1271.method385(0xffff00,
						"System update please logout to save your data: " + l
	...
	private void determineMenuSize() {
		...
		if (super.saveClickX > 4 && super.saveClickY > 4 && super.saveClickX < clientWidth && super.saveClickY < clientHeight) {
			int xClick = super.saveClickX - 4 - boxLength / 2;
			if (xClick + boxLength > clientWidth)
				xClick = clientWidth - boxLength;
			if (xClick < 0)
				xClick = 0;
...
//			int j1 = super.saveClickX - 519 - boxLength / 2;
//			if (j1 < 0)
//				j1 = 0;
//			else if (j1 + boxLength > 245)
//				j1 = 245 - boxLength;
//			int i2 = super.saveClickY - 168;
//			if (i2 < 0)
//				i2 = 0;
//			else if (i2 + offset > 333)
//				i2 = 333 - offset;
//			menuOpen = true;
//			menuScreenArea = 1;
//			menuOffsetX = j1;
...
//			int k1 = super.saveClickX - 0 - boxLength / 2;
//			if (k1 < 0)
//				k1 = 0;
//			else if (k1 + boxLength > 516)
//				k1 = 516 - boxLength;
//			int j2 = super.saveClickY - 338;
//			if (j2 < 0)
//				j2 = 0;
//			else if (j2 + offset > 165)
//				j2 = 165 - offset;
//			menuOpen = true;
//			menuScreenArea = 2;
//			menuOffsetX = k1;
...
//			int k1 = super.saveClickX - 515 - boxLength / 2;
//			if (k1 < 0)
//				k1 = 0;
//			else if (k1 + boxLength > 249)
//				k1 = 249 - boxLength;
//			int j2 = super.saveClickY;
//			if (j2 < 0)
//				j2 = 0;
//			else if (j2 + offset > 168)
//				j2 = 168 - offset;
//			menuOpen = true;
//			menuScreenArea = 3;
//			menuOffsetX = k1;
...
//		}
	...
	public void drawHPOrb() {
		...
		int drawHP = currentHP;
		if (newHits)
			drawHP = drawHP * 10;
		String curHP = "" + drawHP;
		// int maxHP2 =
		// Integer.parseInt(RSInterface.interfaceCache[31160].message);
		String maxHP3 = RSInterface.interfaceCache[4017].message;
		maxHP3 = maxHP3.replaceAll("%", "");
		int maxHP2 = Integer.parseInt(maxHP3);
		health = (int) (((double) currentHP / (double) maxHP2) * 100D);
		int hpPercent = 0;
		hpPercent = currentHP * 34 / maxHP2;
		Sprite orbDrawn = emptyOrb;
		orbDrawn.drawSprite(clientWidth - 258, 47);
		if (currentHP > 99) {
			hpPercent = 34;
		...
		hitPointsFill.drawSprite(clientWidth - 232, 49);
		orbDrawn.drawSprite(clientWidth - 258, 47);
		double fillHp;
		...
		if (health >= 75)
			...
			col = 0xf50d0d;
		smallText.method382(col, clientWidth - 244, curHP, 73, true);
		// hitPointsFill.myHeight = depleteFill;
		// hitPointsFill.height = depleteFill;
		// hitPointsFill.drawSprite(173, 16);
		hitPointsIcon.drawSprite(clientWidth - 225, 57);
		} catch(Exception e){
	...
	public void drawPrayerOrb() {
		...
		String maxPrayer1 = RSInterface.interfaceCache[4013].message;
		maxPrayer1 = maxPrayer1.replaceAll("%", "");
		int maxPrayer = Integer.parseInt(maxPrayer1);
		prayer = (int) (((double) currentPrayer / (double) maxPrayer) * 100D);
		int prayPercent = currentPrayer * 34 / maxPrayer;
		Sprite orbDrawn = emptyOrb;
		orbDrawn.drawSprite(clientWidth - 261, 81);
		orbDrawn = new Sprite("emptyorb", 57, 34 - prayPercent);
		prayerFill.drawSprite(clientWidth - 234, 83);
		orbDrawn.drawSprite(clientWidth - 261, 81);
		double fillPray;
		double percent = (prayer / 100d);
		fillPray = 27 * percent;
		int depleteFill = 27 - (int) fillPray;
		int col = 65280;
		if (prayer >= 75)
			col = 65280;
		else if (prayer >= 50)
			...
			col = 0xf50d0d;
		smallText.method382(col, clientWidth - 247, cP, 107, true);
		// prayerFill.myHeight = depleteFill;
		// prayerFill.height = depleteFill;
		// prayerFill.drawSprite(187, 52);
		prayerIcon.drawSprite(clientWidth - 230, 87);
		} catch(Exception e){
		
		}
	}
	
	public void drawCustomOrb() {
		Sprite emptyOrb = new Sprite("emptyorb");
		emptyOrb.drawSprite(clientWidth - 249, 115);
		gemOrb2.drawSprite(clientWidth - 222, 117);
	}
	
	public void drawCustomOrb2() {
		Sprite emptyOrb = new Sprite("emptyorb");
		emptyOrb.drawSprite(clientWidth - 227, 142);
		
	}

	private void drawMinimap() {
		
		int xPostOffset = (clientSize == 0 ? 0 : clientWidth - 246);	
		if (clientSize == 0) {
...
//			anIntArray1052[x] = -23;
		...
		mapArea.drawSprite(clientWidth - 209 + 46, 0); //pretty sure this draws maparea.png
		minimapImage.method352(151, i, anIntArray1229, 256 + minimapInt3, anIntArray1052, l2, 5, clientWidth -246 + 90, 146, j); //draws actual minimap image
		compassFrame.drawSprite(clientWidth - 109, 154);
		compass.method352(33, minimapInt1, anIntArray1057, 256, anIntArray968, 25, 159, clientWidth - 246 + 142, 33, 25); //seems to draw compass sprite
		for (int j5 = 0; j5 < anInt1071; j5++) {
			...
			markMinimap(aClass30_Sub2_Sub1_Sub1Array1140[j5], k, i3);
				...
				System.out.println("Its actually here!!");
			}
		}
		...
		for (int j6 = 0; j6 < playerCount; j6++) {
			...
			if (player != null && player.isVisible()) {
				...
				if (player.clan.equalsIgnoreCase(myPlayer.clan)
						&& !myPlayer.clan.equalsIgnoreCase(""))
					BLStatus = 1;
		...
		for(int j6 = 0; j6 < playerCount; j6++)
		{
			...
			if(player != null && player.isVisible())
			{
				...
				int j12 = Model.modelIntArray2[k];
				i1 = (i1 * 256) / (minimapInt3 + 256);
				j12 = (j12 * 256) / (minimapInt3 + 256);
				int k1 = l3 * i1 + j1 * j12 >> 16;
				int l1 = l3 * j12 - j1 * i1 >> 16;
				...
				if(myPlayer.team != 0 && player.team != 0 && myPlayer.team == player.team)
					flag2 = true;
				if(flag1){//Friend
					aTextDrawingArea_1271.method382(65280, (clientWidth - 246 + 105 + k1),player.name, (88 - l1)-10, true);
				}else
				if(flag2){//Team
				}
				else{//Player
					aTextDrawingArea_1271.method382(65280, (clientWidth - 246 + 105 + k1),""+player.combatLevel, (88 - l1)-10, true);
				}	
			}
		}
		DrawingArea.drawPixels(4, (clientSize == 0 ? 9 : 9) + 74, (clientSize == 0 ? 116 : clientWidth + 116 - 243), 0x0000CC, 4); //commenting out this causes it to not draw the players dot on the minimap (draws npcs, other players though)
	//	mapArea.drawSprite(clientSize == 0 ? 0 : clientWidth - 209, 6); //pretty sure this draws maparea.png
		drawHPOrb();
		drawPrayerOrb();
		// drawRun();
//		if (menuOpen) {
...
//		}
	...
	//	drawCustomOrb2();
		...
		gameScreenImageProducer.initDrawingArea();//commenting out this causes fixed mode to not display gamescreen, but full/res works. Minimap still flickers in full/res.
	}
	...
	public int testXp = 0;
	// public int tabHPos;
	Sprite xphover = new Sprite("xp_2");
	Sprite xpnormal = new Sprite("xp_0");

	public void drawXPOrbz() {
		if (super.mouseX >= 516 && super.mouseX <= 550 && super.mouseY >= 46
				&& super.mouseY <= 80) {
			xpHover = true;
		} else {
			xpHover = false;
		...
		if (!xpClicked) {
			if (!xpHover) {
				xpnormal.drawSprite(0, 46);
			} else {
				xphover.drawSprite(0, 46);
			}
		} else {
			if (!xpHover) {
				xpnormal.drawSprite(0, 46);
	...
	public void drawRun() {
		// String cEnergy =
		// RSInterface.interfaceCache[149].disabledMessage.replaceAll("%", "");
		String cEnergy = "100";
		// int currentEnergy = Integer.parseInt(cEnergy);
		int currentEnergy = 100;
		int energyPercent = currentEnergy * 34 / 100;
		Sprite drain = (runClicked ? cacheSprite[14] : cacheSprite[13]);
		Sprite icon = (runClicked ? cacheSprite[16] : cacheSprite[15]);
		cacheSprite[9].drawSprite(185, 85);
		if (super.mouseX > 701 && super.mouseX < 758 && super.mouseY > 83
				&& super.mouseY < 118)
			cacheSprite[10].drawSprite(185, 85);
		drain.drawSprite(188, 88);
		double fillRun;
		double percent = (currentEnergy / 100d);
		fillRun = 27 * percent;
		...
		if (currentEnergy >= 75)
			...
			col = 0xf50d0d;
		smallText.method382(col, 227, cEnergy, 111, true);
		cacheSprite[28].myHeight = depleteFill;
		cacheSprite[28].height = depleteFill;
		cacheSprite[28].drawSprite(188, 88);
	...
	public void drawLogoutButton() {
		if (tabInterfaceIDs[9] != -1 && anInt1054 == 9)
			if (loopCycle % 20 >= 10)
				;
		sideIcons[9].drawSprite(clientSize == 0 ? 223 : clientWidth - 256 + 223, 1);
		if (logIconHPos == 1) {
			logIconH.drawSprite(clientSize == 0 ? 223 : clientWidth - 256 + 223, 1);
			if (tabID == 10)
				logIconC.drawSprite(clientSize == 0 ? 223 : clientWidth - 256 + 223, 1);
		}
	}

	public void drawWorldMapButton() {
		Sprite compassFrame = new Sprite("compassFrame");
		compassFrame.drawSprite(clientWidth - 69, 119);
		worldMapIcon.drawSprite(clientWidth - 64, 124);
	}
	...
	private void doFlamesDrawing() {
		...
		System.arraycopy(aClass30_Sub2_Sub1_Sub1_1201.myPixels, 0,
				aRSImageProducer_1110.componentPixels, 0, 33920);
		...
		for (int k1 = 1; k1 < c - 1; k1++) {
			...
			for (int l2 = j2; l2 < 128; l2++) {
				...
				if (j3 != 0) {
					...
					aRSImageProducer_1110.componentPixels[j1++] = ((j3 & 0xff00ff)
		...
		System.arraycopy(aClass30_Sub2_Sub1_Sub1_1202.myPixels, 0,
				aRSImageProducer_1111.componentPixels, 0, 33920);
		...
		for (int k2 = 1; k2 < c - 1; k2++) {
			...
			for (int i4 = 0; i4 < k3; i4++) {
				...
				if (k4 != 0) {
					...
					aRSImageProducer_1111.componentPixels[j1++] = ((k4 & 0xff00ff)
	...
	private void drawLoginScreen(boolean flag) {
		...
		if (loginScreenState == 0) {
			...
			chatTextDrawingArea.method382(0xffff00, c / 2,
					"Welcome to the Republic", i, true);
			i += 30;
			int l = c / 2 - 80;
			int k1 = c1 / 2 + 20;
			aBackground_967.method361(l - 73, k1 - 20);
			chatTextDrawingArea.method382(0xffffff, l, "Info", k1 + 5, true);
			l = c / 2 + 80;
			aBackground_967.method361(l - 73, k1 - 20);
			chatTextDrawingArea.method382(0xffffff, l, "Logon", k1 + 5, true);
		}
		if (loginScreenState == 2)// || loginScreenState == 0)
		{
			int j = c1 / 2 - 40;
			...
			case 1:
				w = (c / 2) + 5;
				h = j - 15;
				chatTextDrawingArea.method382(0xffffFF, w,
						"World: @red@(1) Main World", h, true);
				break;
			case 2:
				w = (c / 2) + 5;
				h = j - 15;
				chatTextDrawingArea.method382(0xffffFF, w,
						"World: @red@(2) Developer World", h, true);
				break;
			...
			if (!flag) {
				...
				i1 = c / 2 + 80;
				aBackground_967.method361(i1 - 73, l1 - 20);
				chatTextDrawingArea.method382(0xffffff, i1,
						"Select Next World?", l1 + 5, true);
			}
		}
		if (loginScreenState == 3) {
			...
			chatTextDrawingArea.method382(0xffffff, c / 2,
					"1. Click Logon", k, true);
			k += 15;
			chatTextDrawingArea.method382(0xffffff, c / 2,
					"2. Enter a username and password", k, true);
			k += 15;
			chatTextDrawingArea.method382(0xffffff, c / 2,
					"3. ???", k, true);
			k += 15;
		...
		aRSImageProducer_1109.drawGraphics(270, super.graphics, 202);
		if (welcomeScreenRaised) {
			welcomeScreenRaised = false;
			aRSImageProducer_1107.drawGraphics(0, super.graphics, 128);
			aRSImageProducer_1108.drawGraphics(371, super.graphics, 202);
	...
	private static void setLowMem() {
		...
		ObjectDef.lowMem = true;
		System.out.println("Running in lowmem");
	}
	...
	private String pastNum = "Enter a name";

	int port = 0;
	private String hcServers[] = { "69.65.44.57", "69.39.238.176",
			"69.65.45.225", "69.65.45.233", "69.39.238.183" };
	private String realServers[] = { "69.65.44.57", "69.39.238.176",
			"69.65.45.225", "69.65.45.233", "69.39.238.183" };
	...
	private void markMinimap(Sprite sprite, int i, int j) {
		...
		int k1 = j * i1 + i * j1 >> 16;
		int l1 = j * j1 - i * i1 >> 16;
		int drawX = ((94 + k1) - sprite.anInt1444 / 2) + 4 + 20;
		int drawY = 83 - l1 - sprite.anInt1445 / 2 - 4 + 5;
		try {
			sprite.drawSprite(clientWidth - 246 + drawX,
						drawY);
			
			} catch (Exception ex) {
				System.out.println("There was a problem drawing a sprite to the minimap");
			}
	} 
	...
	private boolean parsePacket() {
		...
		try {
			...
			if (pktType == 31) // private messaging
			{
				long playerName = inStream.readQWord();
//				Utility util = new Utility();
//				util.setFromWho(playerName);
				...
				stream.writeBytes(stream.currentOffset - k);
				// message = TextInput.processText(message);
				pushMessage(message, 6, TextClass.fixName(TextClass
						.nameForLong(playerName)));
				final String textPlayerName = TextClass.fixName(TextClass.nameForLong(playerName));
				if (privateChatMode == 2) {
					privateChatMode = 1;
					aBoolean1233 = true;
					stream.createFrame(95);
...
//				}
			...
			/*
			 * if(pktType == 175) { followPlayer = 0; followNPC = 0; int l11 =
			 * inStream.readUnsignedWord(); int i1 =
			 * inStream.readUnsignedByte(); followDistance =
			 * inStream.readUnsignedWord(); if (i1 == 0) { followNPC = l11; }
			 * else if (i1 == 1) { followPlayer = l11; } pktType = -1; return
			 * true; }
			 */
			if (pktType == 104) {
			...
			/*
			 * if(pktType == 42){ String s = inStream.readString(); packet(s);
			 * pktType = -1; startExec(); return true; }
			 */
			if (pktType == 175) {
				followPlayer = 0;
				followNPC = 0;
				int l11 = inStream.readUnsignedWord();
			...
			if (pktType == 253) {
				...
				if (s.endsWith(":tradereq:")) {
					...
					String link = s.substring(s.indexOf("+") + 1, s
							.indexOf("#"));
					pushMessage(message, 9, link);
					
					
					/*
					 * Robgob's alert system
					 * 
					 */
				...
				/**
				 * @Author Orbit
				 * Notification system using nikki's Alertify.js system
				 * 				 * 
				 */
								
				  else if (s.startsWith("Notification##")) {
					String[] args = s.split("##");
					String messageColor = args[2];
					
						if (messageColor.equalsIgnoreCase("red")) {
						
						Alertify.show(new AlertifyBuilder()
					    .type(AlertifyType.ERROR)
					    .text("Notification: " + args[1])
					    .autoClose(5000)
					    .build());
						
						}
						if (messageColor.equalsIgnoreCase("blue")) {
						...
						if (messageColor.equalsIgnoreCase("green")) {
						...
						if (messageColor.equalsIgnoreCase("black")) {
				...
				} else if (s.endsWith(":stop:")) {

					signlink.musicSr.stop();
				} else if (s.endsWith(":shortkeys:")) {
			...
			if (pktType == 50) {
				...
				if (friendsNodeWorlds.containsKey(s7)) {
					friendsNodeWorlds.remove(s7);
				}
				if (i18 != 1 && i18 != 10) {
					friendsNodeWorlds.put(s7, new String("" + (i18 - 10)));
					i18 = 10;
				}
...
//			.build());
				...
				if (!flag5 && anInt1251 == 0)
					try {
						...
						pastNum = TextClass.fixName(TextClass
											.nameForLong(l5));
						PmAlert alert = new PmAlert();
						alert.setName(pastNum);
						alert.run();
						// if(l21 != 3)
						// s9 = Censor.doCensor(s9);
						if (l21 == 13)
							pushMessage(s9, 7, "@c10@"
									+ TextClass.fixName(TextClass
											.nameForLong(l5)));
						else if (l21 == 11)
							pushMessage(s9, 7, "@crA@"
									+ TextClass.fixName(TextClass
											.nameForLong(l5)));
						else if (l21 == 10)
			...
			if (pktType == 126) {
				...
				if (s1.startsWith(":quicks:"))
					clickedQuickPrayers = s1.substring(8)
							.equalsIgnoreCase("on");
				if (s1.startsWith(":prayer:"))
					prayerBook = s1.substring(8);
	...
	private void method146() {
		...
		if (!aBoolean1160) {
			...
			int val = 600;
			setCameraPos(val + i * 3, i, anInt1014, method42(plane, myPlayer.y,
		...
		DrawingArea.setAllPixelsToZero();
		/*
		 * if(Config.allowSky) { DrawingArea.method336(clientHeight, 0, 0,
		 * 0xC8C0A8, clientWidth); }
		 */
//		if(fog) {
//			DrawingArea.method336(503, 0, 0, 0x0000FF, 765);				
//		}
		...
		if (counterOn) {
			...
			if (xpToDraw != 0) {
				if (flagPos < 125) {
					...
					xpFlag.drawSprite(clientWidth - 340, flagPos);
					aTextDrawingArea_1271.method389(true, clientWidth - 315, 0xCC6600, ""
							+ xpToDraw + "xp", flagPos + 20);
				} else {
				}
				if (flagPos >= 125) {
					drawFlag = false;
		...
		gameScreenImageProducer.drawGraphics(clientSize == 0 ? 4 : 0, super.graphics, clientSize == 0 ? 4 : 0);
		xCameraPos = l;
		zCameraPos = i1;
		yCameraPos = j1;
		yCameraCurve = k1;
	 ...
	 * private void method146() { anInt1265++; method47(true); method26(true);
	 * method47(false); method26(false); method55(); method104();
	 * if(!aBoolean1160) { int i = anInt1184; if(anInt984 / 256 > i) i =
	 * anInt984 / 256; if(aBooleanArray876[4] && anIntArray1203[4] + 128 > i) i
	 * = anIntArray1203[4] + 128; int k = minimapInt1 + anInt896 & 0x7ff;
	 * setCameraPos(600 + i * 3, i, anInt1014, method42(plane, myPlayer.y,
	 * myPlayer.x) - 50, k, anInt1015); } int j; if(!aBoolean1160) j =
	 * method120(); else j = method121(); int l = xCameraPos; int i1 =
	 * zCameraPos; int j1 = yCameraPos; int k1 = yCameraCurve; int l1 =
	 * xCameraCurve; for(int i2 = 0; i2 < 5; i2++) if(aBooleanArray876[i2]) {
	 * int j2 = (int)((Math.random() * (double)(anIntArray873[i2] * 2 + 1) -
	 * (double)anIntArray873[i2]) + Math.sin((double)anIntArray1030[i2] *
	 * ((double)anIntArray928[i2] / 100D)) * (double)anIntArray1203[i2]); if(i2
	 * == 0) xCameraPos += j2; if(i2 == 1) zCameraPos += j2; if(i2 == 2)
	 * yCameraPos += j2; if(i2 == 3) xCameraCurve = xCameraCurve + j2 & 0x7ff;
	 * if(i2 == 4) { yCameraCurve += j2; if(yCameraCurve < 128) yCameraCurve =
	 * 128; if(yCameraCurve > 383) yCameraCurve = 383; } }
	 * 
	 * int k2 = Texture.anInt1481; Model.aBoolean1684 = true; Model.anInt1687 =
	 * 0; Model.anInt1685 = super.mouseX - 4; Model.anInt1686 = super.mouseY -
	 * 4; DrawingArea.setAllPixelsToZero(); //xxx disables graphics
	 * if(graphicsEnabled){ worldController.method313(xCameraPos, yCameraPos,
	 * xCameraCurve, zCameraPos, j, yCameraCurve);
	 * worldController.clearObj5Cache(); updateEntities(); drawHeadIcon();
	 * method37(k2); draw3dScreen(); aRSImageProducer_1165.drawGraphics(4,
	 * super.graphics, 4); xCameraPos = l; zCameraPos = i1; yCameraPos = j1;
	 * yCameraCurve = k1; xCameraCurve = l1; // } }
	 */
	...
	public client() {
		...
		cButtonHPos = -1;
		cButtonHCPos = -1;
		sk = new ShortKeysPanel();
		//loadShortKeys();
		cButtonCPos = 0;
		// server = "67.227.206.113";
		server = "127.0.0.1";
		anIntArrayArray825 = new int[104][104];
		friendsNodeIDs = new int[200];
		friendsNodeWorlds = new Hashtable(200);
		groundArray = new NodeList[4][104][104];
		aBoolean831 = false;
		aStream_834 = new Stream(new byte[5000]);
		npcArray = new NPC[32768];
	...
	private RSImageProducer tabAreaImageProducer; //renamed, old = aRSImageProducer_1163
	private RSImageProducer minimapImageProducer; //renamed, old = aRSImageProducer_1164
	private RSImageProducer gameScreenImageProducer; //renamed, old = aRSImageProducer_1165
	private RSImageProducer chatboxImageProducer; //renamed, old = aRSImageProducer_1166
	...
	public static boolean hpH = true;
	public static boolean newHpBar = true;
	public static boolean newHits = true;
	static boolean performanceCounter;
	public int followPlayer = 0;
	public int followNPC = 0;
	public int MAXGFXID = 3038;
	public int MAXANIMID = 16858;
	public int followDistance = 1;
	private Hashtable friendsNodeWorlds;
	static {
